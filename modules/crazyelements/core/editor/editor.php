		...
		switch ( $hook ) {
			...
			case 'prdlayouts':
				...
				if(isset( $_SERVER['HTTP_REFERER'])){
					...
					$exit_to_dashboard = $context->link->getAdminLink( 'AdminCrazyPrdlayouts' );
				}
				break;
			case 'extended':
			...
			default:
					$preview           = PrestaHelper::setPreviewForHook( $hook );
					$preview_w_qs      = PrestaHelper::setPreviewForHook( $hook ) . '?hook=' . $hook . '&id=' . $id . '&id_lang=' . $id_lang . '&id_shop=' . $id_shop . '&token=' . $token . PrestaHelper::$disable_activity . '&preview=1';
					if(isset( $_SERVER['HTTP_REFERER'])){
						$exit_to_dashboard = $_SERVER['HTTP_REFERER'];
					}else{
						$exit_to_dashboard = $context->link->getAdminLink( 'AdminDashboard' );
