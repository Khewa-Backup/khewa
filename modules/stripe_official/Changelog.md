...
## 3.7.3 - 2026-05-11
- Fix security issue in Express Checkout where sensitive authentication data could be exposed in the response (CONNECTORS-2960)

## 3.7.2 - 2026-03-20
- Improved user experience while switching payment methods, and loading of payment element (CONNECTORS-2577)
- Improve error handling on admin order detail page for Stripe related details (CONNECTORS-2571)
- Trigger payment element if not triggered automatically on page load (CONNECTORS-2734)
- Improved confirmation token creation  (CONNECTORS-2699)
- Refresh payment method elements after product changes (CONNECTORS-2709)
- Format amount for Apple Pay (CONNECTORS-2627)
- make handleOrderAction idempotent for checkout session flow (CONNECTORS-2649)
- Update orders when PrestaShop split orders (CONNECTORS-2665)
- Add missing parameter in checkout.js (CONNECTORS-2634)
- Improve amount calculation for Express Checkout when cart rules are in place (CONNECTORS-2668)
- Make it possible to disable payment methods in Express Checkout Element (CONNECTORS-2743)

## 3.7.1 - 2026-03-02
- Security Issue fixed for 3ds cards. (CONNECTORS-2723)

## 3.7.0 - 2026-02-11
- Added Payment Method Messaging Element (CONNECTORS-1525)
- Payment Method Messaging Element discrepancy (CONNECTORS-1788)
- Added Klarna as an Express Checkout option on config page (CONNECTORS-1707)
- Added description for Extended Logging setting on config page (CONNECTORS-2005)

## 3.6.9 - 2026-01-26
- Set backorder statuses (paid or not paid) when setting is enabled (CONNECTORS-1983)
- Show Express Checkout button for out of stock products (CONNECTORS-1983)
- Resolved error from the logs related to the Product Lazy Array (CONNECTORS-1871)
- Fixed validation of the cart from webhook (CONNECTORS-2173)
- Compatibility with PIX payment method (CONNECTORS-2169)
- Compatibility with PrestaShop Hummingbird theme (CONNECTORS-1915)
- Save Payment method at customer level (CONNECTORS-2012)
- Investigate webhook error on save (CONNECTORS-2153)
- Clear cart if webhook finalize the order (CONNECTORS-1910)
- Improve update stripe intent for legacy option (CONNECTORS-1716)
- Add order reference for failed payment using the Express Checkout (CONNECTORS-2200)
- Fix customer redirect when the order was created from webhook and use the Legacy flow (CONNECTORS-2212)
- Fix full payment byPass (CONNECTORS-2589)
- Do not send email on order creation with awaiting payment status (CONNECTORS-2603)
- Fix payment error on accessing handleNextAction twice (CONNECTORS-2602)
- Fix security issue with payment detail name override (CONNECTORS-2591)
- OrderFailure page secured with additional hash key (CONNECTORS-2633)
- Improve security by securing order confirmation link with cart secure key (CONNECTORS-2590)
