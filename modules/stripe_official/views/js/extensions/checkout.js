...
window.onerror = async function(msg, url, line, col, error) {
  try {
    ...
    const text = `${msg || ""} ${url || ""} ${stack} ${String(error || "")}`.toLowerCase();

    if (
      !text.includes("stripe") &&
      !text.includes("checkout.js") &&
      !text.includes("expresscheckout.js")
    ) {
      return;
    }

    let msg = stack || String(msg || "Unknown Stripe error");
    if (msg.length > 1000) msg = msg.slice(0, 999) + "…";

    // minimal dedupe (per page load)
    if (sent.has(msg)) return;
    sent.add(msg);
...
function setPaymentOptionsLoading(loading) {
  const stripeInput = document.querySelectorAll('input[name="payment-option"][data-module-name="stripe_official"]');
  const stripeLabel = document.querySelector('label[for="' + stripeInput[0].id + '"]');
  ...
  if (loading) {
    ...
    loadingInterval = setInterval(() => {
      dots = (dots + 1) % 4;
      stripeLabel.textContent = stripeLabel.textContent.replace(/[.\s]+$/, '') + '.'.repeat(dots);
    }, 200);
  } else {
    clearTimeout(loadingTimeout);
    clearInterval(loadingInterval);
    stripeLabel.textContent = stripeLabel.textContent.replace(/[.\s]+$/, '')
  }
  document.querySelectorAll('input[name="payment-option"]').forEach(input => input.disabled = loading);
}
...
$(function() {
  ...
  if (paymentFormInput && paymentFormInput.checked) {
    triggerTimeout = setTimeout(function () {
      const stripeOption = document.querySelector('input[name="payment-option"][data-module-name="stripe_official"]');
      if (stripeOption) {
        stripeOption.dispatchEvent(new Event('change', { bubbles: true }));
      }
    }, 5000);
  }
...
//Create elements on the payment page
  ...
  let $classSelector = document.querySelector('.payment-options ') || document.querySelector('.payment__list');
  let $paymentForm = document.querySelector('#js-stripe-payment-form');
  $paymentForm.style = '';
  let paymentFormInput = document.querySelector('input[data-module-name="stripe_official"]');
...
//When the customer presses “Submit”, create the PaymentIntent server-side and confirm client-side
  ...
  try {
    const confirmationTokenParams = await stripe.createConfirmationToken({
      elements,
      params: { payment_method_data: createBillingDetails },
    });

    if (confirmationTokenParams.error) {
      handleError(confirmationTokenParams.error);
      return;
    }

    const confirmationTokenId = confirmationTokenParams.confirmationToken?.id;
    if (!confirmationTokenId) {
      handleError(new Error("Stripe did not return confirmationToken.id"));
      return;
    }

    const customer = String(createElementsDetails?.customer ?? "");
    const verifyToken = await sha256(`${customer}${confirmationTokenId}`);


    const url = new URL(handle_order_action_url, window.location.href);
    url.searchParams.set("confirmationToken", confirmationTokenId);
