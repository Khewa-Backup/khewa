...
$(function() {
    ...
    prestashop.on('updatedProduct', (event) => {
      ...
      if ((productQuantity === '0' && stripe_product_out_of_stock !== 1) || (parseInt(quantity) > parseInt(productQuantity) && stripe_product_out_of_stock !== 1)) {
        if (expressCheckoutElement) {
          console.log('Unmounting express checkout element => Product out of stock');
  ...
  if (typeof prestashop !== 'undefined' && typeof prestashop.on === 'function') {
    prestashop.on('updateCart', (args) => {
      if (typeof args != 'undefined' && typeof args.resp != 'undefined' && (args.resp.errors !== '' || args.resp.cart.products.length === 0)) {
        if (expressCheckoutElement) {
          console.log('Unmounting express checkout element => Cart has errors or is empty');
          expressCheckoutElement.unmount();
        }
      } else {
        initialize();
  ...
  if (express_checkout === '0' || typeof stripe_express_amount == 'undefined' || stripe_express_amount === 0 || typeof restrictedExpressCheckout == 'undefined' || restrictedExpressCheckout === true) {
      return;
  }
...
async function initialize() {
  ...
  if(document.body.id === 'product' && locations.includes('product')) {
    ...
    $classSelector = document.querySelector('.cart-detailed-totals') || document.querySelector(".cart-detailed__totals");
  ...
  let elementOptions = {
    buttonType: {
      ...
      klarna: klarna_button_type ? klarna_button_type : 'continue',
    },
    buttonTheme: {
      ...
      klarna: klarna_button_theme ? klarna_button_theme : 'dark',
    },
    paymentMethods: {
      applePay: enable_apple_pay === '1' ? 'always' : 'never',
      googlePay: enable_google_pay === '1' ? 'always' : 'never',
      paypal: enable_pay_pal === '1' ? 'auto' : 'never',
      klarna: enable_klarna_express === '1' ? 'auto' : 'never',
      amazonPay: enable_amazon_pay === '1' ? 'auto' : 'never',
      link: enable_link_express === '1' ? 'auto' : 'never'
    }
  }
  ...
  {
    console.log('Unmounting express checkout element => Not on the right page');
    expressCheckoutElement.unmount();
  }
  ...
  expressCheckoutElement.on('click', (event) => {
    ...
    if(document.body.id === 'product')
    {
      ...
      let productAmount = amountProductStringElement ? parseLocalizedAmountToNumber(amountProductStringElement.textContent) : 0;
      amount = productAmount * parseInt(productQuantity);
      elements.update({amount: amount});
    }
    if (document.body.id === 'cart') {
      let cartTotalStringElement = document.querySelector('.cart-total .value') || document.querySelector('.cart-total .cart-summary__value');
      let productTotalAmount  = cartTotalStringElement ? parseLocalizedAmountToNumber(cartTotalStringElement.textContent) : 0;

      //get shipping amount
      let shippingElement = document.querySelector('#cart-subtotal-shipping .value');
      let shippingTotal = 0;
      if (shippingElement) {
        shippingTotal = parseLocalizedAmountToNumber(shippingElement.textContent);
      }
  ...
  expressCheckoutElement.on('ready', (params) => {
    if (!params.availablePaymentMethods) {
      console.error('Stripe Module Express Checkout: No available payment methods information received. Please check payment method settings Stripe account');
      return;
    }
    const isAnyAvailable = Object.keys(params.availablePaymentMethods).some((key) => params.availablePaymentMethods[key] === true);
    if (!isAnyAvailable) {
      console.warn('Stripe Module Express Checkout: No available payment methods. Please check payment method settings Stripe account', params.availablePaymentMethods);
    }
  });
}
...
function parseLocalizedAmountToNumber(raw) {
  let s = String(raw)
