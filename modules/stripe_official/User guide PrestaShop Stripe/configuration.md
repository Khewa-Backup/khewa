...
3. Configure the Stripe Connector for PrestaShop:
  ...
  - [Enable extended logging](/connectors/prestashop/configuration#extended-logging)
  - [Connect to Cloud Sync and PrestaShop Account](/connectors/prestashop/configuration#CloudSync-PrestaShopAccount)
...
## Save customer payment methods {% #payment-methods %}
...
* **Off** – Payment methods are not saved*.

* [**On session**](https://support.stripe.com/questions/what-is-the-difference-between-on-session-and-off-session-and-why-is-it-important) – Customers are asked at checkout if they want to save their payment details for future use.

* **Off session** – Customers are asked at checkout if they want to save their payment details for future use. They will also be notified that by providing card information, they authorize future charges per your Terms.
  Useful for scenarios like subscription renewals, late fees, incidentals (e.g. car rentals), or MOTO payments.

*Customers can still use Link to save their payment methods globally.
...
## Enable Extended Logging {% #extended-logging %}

You can check **Extended Logging** to capture detailed activity for easier debugging. When unchecked, the module records standard logs only.

## Connect to Cloud Sync and PrestaShop Account {% #CloudSync-PrestaShopAccount %}

You can now connect Stripe to **Cloud Sync** and **PrestaShop Account**, which PrestaShop strongly recommends to enhance your store's functionality and integration.

**Cloud Sync** ensures your store's data is securely backed up and synchronized with PrestaShop's cloud, enabling centralized management and real-time performance monitoring.

1. On the **Stripe Configure page**, click the **Link** button in the **Link your store to a PrestaShop account** section.

2. In the newly opened window, click **Accept and associate**.

3. Click **Continue the installation**. After this step, a green checkmark will appear next to the section title, along with a message confirming that your store is linked to the PrestaShop account.

**PrestaShop Account** acts as a central hub for managing integrations, licensing, and access to PrestaShop’s ecosystem and Addons Marketplace services.

1. On the **Stripe Configure page**, click the **Share my data** button in the **Share your data with Stripe payment module** section.

2. In the opened window, click **Allow**. Data synchronization will be confirmed by a green checkmark and a confirmation message in the section.

If you prefer not to install **CloudSync**,  you can click the **Skip** button at the top of the configuration page. You can always install it later by using the **If you want to use CloudSync with the module, please install the dependencies here** option.

**Note:** On Preshashop 1.7 with **multishop** enabled and **All shops** selected, the dependency installation dialog may persist. To proceed, either click **Skip**  or select a specific shop to access the app configuration.
...
## Payment Method Messaging Element {% #messaging-element %}

The **Payment Method Messaging Element** is a dynamic UI component that informs customers about available **Buy Now, Pay Later (BNPL)** options before they reach checkout.

* **Functionality:** It automatically displays localized installment plans based on the customer's location, currency, and the specific order total.
* **Placement:** Displayed on the **Product Page** and the **Cart Page**.
* **Design:** Inherits your site's theme automatically for a seamless look and feel.
