...
class StripeCheckoutSessionService
{
    ...
    public function retrieveLastOpenCheckoutSessionForCartId($cartId): ?Session
    {
        // Get idempotency key for cart, and validate of that is a real checkout session
        $idempotencyKey = StripeIdempotencyKey::getOrCreateIdempotencyKey($cartId);
        if (!$idempotencyKey || !$idempotencyKey->id_payment_intent || strpos($idempotencyKey->id_payment_intent, 'cs_') === false) {
            throw new Exception('No valid idempotency key or payment intent found for cart id ' . $cartId);
        }
        // retrieve checkout session from stripe, check if it is still open
        $checkoutSession = $this->getStripeCheckoutSession($idempotencyKey->id_payment_intent);
        if (!$checkoutSession || $checkoutSession->status != 'open') {
            throw new Exception('No valid checkout session found for payment intent ' . $idempotencyKey->id_payment_intent);
        }
