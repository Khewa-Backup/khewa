...
class StripePaymentIntentService
{
    ...
    public function updateStripePaymentIntent($paymentIntent, $orderReference)
    {
        ...
        $paramsToApply = $this->needsPiUpdate($paymentIntent, $orderReference, $shippingDetails);

        $stripePaymentIntent = null;
        if ($paramsToApply) {
            try {
                $stripePaymentIntentOptions = $this->buildPaymentIntentIdempotencyKeyUpdate($paymentIntent->metadata->id_cart);
                $stripePaymentIntent = $this->stripeClient->paymentIntents->update($paymentIntent->id, $updateIntentParams, $stripePaymentIntentOptions);
            } catch (ApiErrorException $e) {
    ...
    public function buildPaymentIntentIdempotencyKeyUpdate($cartId)
    {
        $idempotencyKey = StripeIdempotencyKey::getOrCreateIdempotencyKey($cartId);

        return [
            'idempotency_key' => $idempotencyKey->idempotency_key . uniqid() . StripePaymentIntent::PAYMENT_INTENT_UPDATE,
        ];
    }

    /**
     * Compare PaymentIntent fields (description + shipping) ignoring key order.
     * Returns an array of fields that actually changed and need updating.
     *
     * @param PaymentIntent $pi
     * @param string|null $newDesc
     * @param array|null $newShipping
     *
     * @return array Fields that differ
     */
    public function needsPiUpdate(PaymentIntent $pi, ?string $newDesc, ?array $newShipping): array
    {
        $diff = [];

        // --- Description comparison ---
        $currentDesc = trim((string) ($pi->description ?? ''));
        $targetDesc = trim((string) ($newDesc ?? ''));

        if ($targetDesc !== '' && $targetDesc !== $currentDesc) {
            $diff['description'] = $targetDesc;
        }

        // --- Shipping comparison ---
        if ($newShipping) {
            StripeProcessLogger::debugLog('needsPiUpdate - newShipping ' . json_encode($newShipping));
            // Convert to canonical form: sorted keys recursively
            ...
            $existingShip = $pi->shipping ? $pi->shipping->toArray() : [];
            $normExisting = $normalize($existingShip);
            $normTarget = $normalize($newShipping);

            // Compare via JSON since order is normalized
            if (json_encode($normExisting, JSON_UNESCAPED_UNICODE) !== json_encode($normTarget, JSON_UNESCAPED_UNICODE)) {
                StripeProcessLogger::debugLog('Compare via JSON since order is normalized ' . json_encode($newShipping));
                $diff['shipping'] = $newShipping;
            }
        }
    ...
    public function getCartIdByPaymentIntentId(string $paymentIntentId): int
    {
        if (empty($paymentIntentId)) {
            return 0;
        }

        $sql = 'SELECT `id_cart` FROM `' . _DB_PREFIX_ . 'stripe_payment`
            WHERE `id_payment_intent` = "' . pSQL($paymentIntentId) . '"';
        ...
        if ($cartId <= 0) {
            ...
            if (!empty($psIdempotencyKey->id_cart)) {
                $cartId = $psIdempotencyKey->id_cart;
            }
        }

        return ($cartId > 0) ? (int) $cartId : 0;
    }
}
