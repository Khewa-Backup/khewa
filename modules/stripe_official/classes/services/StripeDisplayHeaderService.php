...
class StripeDisplayHeaderService
{
    ...
    {
        $this->module->triggerHook('actionStripeModifyOrderPageNames', ['orderPageNames' => &$this->orderPageNames]);
        ...
        if ($_GET['controller'] === 'product' && version_compare(_PS_VERSION_, '1.7.2.4', '>')) {
            $productLazyArray = null;
            try {
                $productLazyArray = StripeProductLazyArrayFactory::createProductLazyArrayFromProductId($this->context, $productId);
            } catch (\Throwable $e) {
                StripeProcessLogger::logError('Cannot get ProductLazyArray from ID: ' . $e->getMessage() . ' - ' . $e->getTraceAsString(), 'stripe_official');
            }
            $this->module->triggerHook('displayProductActions', !empty($productLazyArray) ? ['product' => $productLazyArray] : ['product' => new \Product($productId)]);
        }
        if ($_GET['controller'] === 'cart' && version_compare(_PS_VERSION_, '1.7.2.4', '>')) {
            $this->module->triggerHook('displayExpressCheckout');
        }
        ...
        \Media::addJsDef([
            ...
            'klarna_button_theme' => \Configuration::get(\Stripe_official::KLARNA_BUTTON_THEME),
            'klarna_button_type' => \Configuration::get(\Stripe_official::KLARNA_BUTTON_TYPE),
            ...
            'enable_apple_pay' => \Configuration::get(\Stripe_official::ENABLE_APPLE_PAY),
            'enable_google_pay' => \Configuration::get(\Stripe_official::ENABLE_GOOGLE_PAY),
            'enable_pay_pal' => \Configuration::get(\Stripe_official::ENABLE_PAY_PAL),
            'enable_klarna_express' => \Configuration::get(\Stripe_official::ENABLE_KLARNA_EXPRESS),
            'enable_amazon_pay' => \Configuration::get(\Stripe_official::ENABLE_AMAZON_PAY),
            'enable_link_express' => \Configuration::get(\Stripe_official::ENABLE_LINK_EXPRESS),
    ...
    private function registerJs()
    {
        ...
        if (version_compare(_PS_VERSION_, '1.7', '>=')) {
            ...
            if ($controller === 'product' || $controller === 'cart') {
                $this->context->controller->registerJavascript(
                    $this->module->name . '-stripe-basil',
                ...
                if (_PS_USE_MINIFY_JS_) {
                    $this->context->controller->registerJavascript(
                        ...
                        'modules/' . $this->module->name . '/views/js/paymentMsgElement.js',
                        ['version' => $jsVersion]
                    );
                } else {
                    $this->context->controller->registerJavascript(
                        $this->module->name . '-payments',
                        'modules/' . $this->module->name . '/views/js/extensions/paymentMsgElement.js',
                        ['version' => $jsVersion]
                    );
                }
            }
    ...
    {
        $jsVersion = hash('sha256', $this->module->version . \Stripe_official::getPublishableKey());
