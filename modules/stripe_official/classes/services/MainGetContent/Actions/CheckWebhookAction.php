...
class CheckWebhookAction extends BaseAction
{
    public function execute()
    {
        ...
        if (\Shop::isFeatureActive()) {
            Stripe::setApiKey($this->module->getSecretKey($this->getShopGroupId(), $this->getShopId()));
        }
        if ($webhookId && \Stripe_official::isWellConfigured()) {
            ...
            try {
                ...
                StripeProcessLogger::logWarning("CheckWebhookAction error ShopGroup: {$this->getShopGroupId()} Shop: {$this->getShopId()} => " . $e->getMessage(), 'CheckWebhookAction');
