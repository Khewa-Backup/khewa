...
class RegisterCloudSyncAction extends BaseAction
{
    ...
    {
        if (!$this->module->isCloudSyncEnabled()) {
            return true;
        }
        ...
        if (!$mboInstaller->areDependenciesMet()) {
            if (
                !$moduleManager->isInstalled('ps_mbo')
                || !$moduleManager->isEnabled('ps_mbo')
            ) {
                $this->module->getContext()->controller->warnings[] = $this->module->getTranslationService()->translate('To use Cloud Sync, please install and enable the "PrestaShop Marketplace" module first.');
