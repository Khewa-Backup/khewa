...
**5. pdf/footer.tpl:**
Open core `pdf/footer.tpl` and add a marker line like `TEST123`. Generate any invoice PDF from BO → Orders. The PDF must show the TPS/TVQ numbers and must **NOT** show `TEST123` — proof the theme copy (`themes/warehouse/pdf/footer.tpl`) is used, not the core file. Remove the marker.
...
Add a marker `TEST123` to core `pdf/invoice.total-tab.tpl`. Download the invoice PDF of an order that used a fixed-amount/gift-card discount. Totals must be correct (Total Discounts, Total Tax lines present) and no `TEST123` — the theme copies win. Remove the marker.
...
**14. Remove Specific References:** Not done — waiting for confirmation of what the card means.
