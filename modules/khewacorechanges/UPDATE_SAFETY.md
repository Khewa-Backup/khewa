...
| 11 | Disabling mail send from core | Two-part fix: (a) `src/Core/Stock/StockManager.php` — core ITSELF emails employees "Product out of stock" with no module involved; that `Mail::Send` call was commented out (lines ~319-333; this is the "search Mail::send( in src" from the card). (b) The old `mailalerts` folder was renamed to `mailalerts_old`. | The module's `hookActionEmailSendBefore` blocks the core StockManager `productoutofstock` email (matched by its `src/Core/Stock` template path — ps_emailalerts' own stock alerts untouched). **Core can be updated; the block keeps working.** The folder rename needs nothing. | **HANDLED** |
...
**1. Rock (RockPOS):** Not handled by choice — the module does not touch or snapshot RockPOS. Nothing to test here; before any RockPOS vendor update, diff and merge the customised files by hand.
...
Update-proof test: restore the commented-out `Mail::Send` block in `src/Core/Stock/StockManager.php` (lines ~319-333, uncomment it), then trigger a product going out of stock. Employees must STILL get no core "Product out of stock" email — the module blocks it at `Mail::send` level. Re-comment the block (or leave it uncommented — the hook covers it either way). ps_emailalerts' own stock alerts must still work. And still: never rename `modules/mailalerts_old/` back.
...
| `files/root/…` | Byte-for-byte copies: admin customer-thread `view.tpl` (#6), `ps_emailalerts.php` (#16). No new code — snapshots for Apply. |
