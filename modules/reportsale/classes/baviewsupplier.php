...
class BaViewSupplier extends ReportSale
{
    ...
    public function insertsupplier($product, $order)
    {
        ...
        if (!empty($data)) {
            ...
            $query = 'UPDATE ' . _DB_PREFIX_ . 'ba_report_supplier SET total_quantity="' . (int) $total_quantity . '",'
                    ...
                    . 'total_cost="' . (float) $total_cost . '",gross_profit="' . (float) $gross_profit . '",'
        ...
        if (empty($data)) {
            Db::getInstance()->insert('ba_report_supplier', [
    ...
    public function updateAllSupplierReport()
    {
        ...
        for ($i = 0; $i < $n; ++$i) {
            $get_data = $data[$i];
            $total_prd = (float) $get_data['total_products_no_tax'];
            $of_total_sales = ($total_prd / (float) $this->gettotalproducnotax()) * 100;
            $gross_profit = (float) $get_data['gross_profit'];
            $of_total_gross_profits = ($gross_profit / (float) $this->gettotalgrossprofits()) * 100;
            ...
            $query = 'UPDATE ' . _DB_PREFIX_ . 'ba_report_supplier SET '
                    ...
                    . 'net_margin="' . (float) $net_margin . '",gross_margin="' . (float) $gross_margin . '" '
