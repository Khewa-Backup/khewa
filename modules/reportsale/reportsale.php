...
class ReportSale extends Module
{
    ...
    public function getContent()
    {
        ...
        /********This Tab Product********* */
        if ($tabvalue == 'product') {
            $list_category = $this->getAllCategoriesName();
            ...
            $list_manufacturers = $Manufacturer->getManufacturers();
            ...
            $list_supplier = $supplier->getSuppliers();
    ...
    {
        $range_orderdate = trim(Tools::getValue($prefix . 'range_orderdate'));
        $range_invoicedate = trim(Tools::getValue($prefix . 'range_invoicedate'));
        $range_deliverydate = trim(Tools::getValue($prefix . 'range_deliverydate'));
        $range_creditdate = trim(Tools::getValue($prefix . 'range_creditdate'));
        ...
        Configuration::updateValue($prefix . 'range_orderdate', $range_orderdate);
        Configuration::updateValue($prefix . 'range_invoicedate', $range_invoicedate);
        Configuration::updateValue($prefix . 'range_deliverydate', $range_deliverydate);
        Configuration::updateValue($prefix . 'range_creditdate', $range_creditdate);
    ...
    public function getConfiguration($prefix, $product = false)
    {
        ...
        $arr_configuration = [
            ...
            'range_orderdate' => Configuration::get($prefix . 'range_orderdate'),
            'range_invoicedate' => Configuration::get($prefix . 'range_invoicedate'),
            'range_deliverydate' => Configuration::get($prefix . 'range_deliverydate'),
            'range_creditdate' => Configuration::get($prefix . 'range_creditdate'),
        ];
    ...
    public function basicdata()
    {
        ...
        foreach ($result as $key1 => $v) {
            ...
            $result[$key1]['ecluding_ecotax_tax_amount'] = $this->formatPriceInCSV($ecluding_ecotax_tax_amount, $a);
            ...
            if (empty($v['invoice_number'])) {
                $result[$key1]['invoice_number'] = '';
            } else {
                $result[$key1]['invoice_number'] = $this->displayInvoiceNumber($v['invoice_number'], $v);
            }
    ...
    public function calcDiscount($product)
    {
        ...
        if ($product['reduction_percent'] > 0) {
            $unit_price_before_discount_excl_tax = $product['original_product_price'];
    ...
    public function convertProductToDefaultCurrenct($product, $c_from)
    {
        ...
        $product['group_reduction'] = (float) $product['group_reduction'];
