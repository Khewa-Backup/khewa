			...
			<div class="tab-pane {if !empty($aExclusionRules)}active{/if}" id="advanced">
				...
				<div class="form-group">
					...
					<button class="btn btn-success "
						onclick="check = confirm('{l s='Are you sure you want to activate the selected rules?' mod='gmerchantcenterpro'}');if(!check)return false;iRulesId = oGmcPro.getBulkCheckBox('bt_rules-box', false);$('#loadingGoogleDiv').show();oGmcPro.hide('bt_feed-settings-exclusion');oGmcPro.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.rulesList.action}&sType={$aQueryParams.rulesList.type}&iRuleId='+iRulesId+'&sUpdateType=bulk&bActivate=1&sDisplay=exclusion', 'bt_feed-settings-{$sDisplay}', 'bt_feed-settings-{$sDisplay}', null, null, 'loadingGoogleDiv');"><i
							class="icon icon-cogs"></i><span>&nbsp;{l s='Activate selection' mod='gmerchantcenterpro'}</span>
					</button>
					&nbsp;-&nbsp;
					<button class="btn btn-warning "
						onclick="check = confirm('{l s='Are you sure you want to deactivate the selected rules?' mod='gmerchantcenterpro'}');if(!check)return false;iRulesId = oGmcPro.getBulkCheckBox('bt_rules-box', false);$('#loadingGoogleDiv').show();oGmcPro.hide('bt_feed-settings-exclusion');oGmcPro.ajax('{$sURI|escape:'htmlall':'UTF-8'}', '{$sCtrlParamName|escape:'htmlall':'UTF-8'}={$sController|escape:'htmlall':'UTF-8'}&sAction={$aQueryParams.rulesList.action}&sType={$aQueryParams.rulesList.type}&iRuleId='+iRulesId+'&sUpdateType=bulk&bActivate=0&sDisplay=exclusion', 'bt_feed-settings-{$sDisplay}', 'bt_feed-settings-{$sDisplay}', null, null, 'loadingGoogleDiv');"><i
		...
		<ul class="nav nav-tabs" id="myTab"">
			...
			<div class="tab-content" id="myTabContent">
				...
				<div class="tab-pane" id="free-shipping">
					...
					<div class="form-group">
						<label class="control-label col-xs-12 col-md-3 col-lg-3">
							<span class="label-tooltip" title="{l s='If the product price without tax is over the value, the shipping cost will be free' mod='gmerchantcenterpro'}"><b>{l s='Set free shipping cost for product price greater than' mod='gmerchantcenterpro'}</b></span></label>
						<div class="col-xs-12 col-md-1 col-lg-1">
							<input type="text" size="5" name="bt_free_shipping_price" value="{if !empty($freeShippingPrice)}{$freeShippingPrice|floatval}{/if}" />
						</div><p>{l s='Tax excluded, before discount' mod='gmerchantcenterpro'}</p>
						&nbsp;
						<span class="icon-question-sign label-tooltip" title="{l s='If the product price without tax is over the value, the shipping cost will be free' mod='gmerchantcenterpro'}"></span>
					</div>
