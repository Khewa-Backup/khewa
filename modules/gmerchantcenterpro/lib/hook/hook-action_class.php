...
class BT_GmcProHookAction extends BT_GmcProHookBase
{
    ...
    private function updateOrderStatus(array $aParams = null)
    {
        if (!isset($_POST['source'])) {
            ...
            $sGsaOrderId = BT_GmcpGsaDao::getGsaOrderFromPsOrder((int) $aParams['id_order']);

            if (!empty($sGsaOrderId)) {
                //Manage case paid
                if (!BT_GmcpGsaDao::updateGsaOrders('is_paid', (int) $aParams['newOrderStatus']->paid, (string) $sGsaOrderId)) {
                    throw new Exception("Update paid status failed", 100);
                } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'paid', (string) $sGsaOrderId)) {
                    throw new Exception("Update paid status failed", 101);
                }

                // Manage case shipped
                if (!empty($aParams['newOrderStatus']->shipped)) {
                    if (!BT_GmcpGsaDao::updateGsaOrders('is_shipped', (int) $aParams['newOrderStatus']->shipped, (string) $sGsaOrderId)) {
                        throw new Exception("Update shipped status failed", 101);
                    } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'shipped', (string) $sGsaOrderId)) {
                        throw new Exception("Update shipped status failed", 101);
                    }
                }

                //Manage delivered orders only if the delivered status sync is ok
                $bIsShipped = BT_GmcpGsaDao::isOrderShippedSync($sGsaOrderId, 1);
                if (!empty($bIsShipped)) {
                    if (!BT_GmcpGsaDao::updateGsaOrders('is_delivered', (int) $aParams['newOrderStatus']->delivery, (string) $sGsaOrderId)) {
                        throw new Exception("Update delivery status failed", 101);
                    } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'delivered', (string) $sGsaOrderId)) {
                ...
                if (!empty($bPrepared)) {
                    if (!BT_GmcpGsaDao::updateGsaOrders('is_prepared', (int) $bPrepared, (string) $sGsaOrderId)) {
                        throw new Exception("Update preparation status failed", 101);
                    } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'preparation', (string) $sGsaOrderId)) {
                        throw new Exception("Update shipped status failed", 101);
                    }
                }
                ...
                if (!empty($bRefunded)) {
                    if (!BT_GmcpGsaDao::updateGsaOrders('is_refunded', (int) $bRefunded, (string) $sGsaOrderId)) {
                        throw new Exception("Update refunded status failed", 102);
                    } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'refunded', (string) $sGsaOrderId)) {
                        throw new Exception("Update refunded status failed", 101);
                    }
                }

                //Check if there is order lines to refund
                $aProductLinesToRefund = BT_GmcpGsaDao::haveOrderLineToRefund((int) $aParams['id_order']);
                // If we find some product line to refund
                if (!empty($aProductLinesToRefund)) {
                    //We update the module table column gsa_orders_data
                    if (!BT_GmcpGsaDao::updateGsaOrders('is_product_refunded', 1, (string) $sGsaOrderId)) {
                        throw new Exception("Update product refunded status failed", 102);
                    } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'partial refund', (string) $sGsaOrderId)) {
                        ...
                        if (is_array($aProductLinesToRefund) && isset($aProductLinesToRefund)) {
                            ...
                            foreach ($aProductLinesToRefund as $aProductData) {
                                if (!BT_GmcpGsaDao::updateGsaOrdersProducts('quantity_refunded', (int) $aProductData['product_quantity_refunded'], (string) $sGsaOrderId, (int) $aProductData['product_id'],  (int) $aProductData['product_attribute_id'])) {
                ...
                $aProductLinesToReturn = BT_GmcpGsaDao::haveOrderLineToReturn((int) $aParams['id_order']);
                ...
                foreach ($aReturnedData as $aData) {
                    ...
                    if (!empty($aProductLinesToReturn) && $aData['sate'] == 1) {
                        //We update the module table column gsa_orders_data
                        if (!BT_GmcpGsaDao::updateGsaOrders('is_returned', 1, (string) $sGsaOrderId)) {
                            throw new Exception("Update product return status failed", 102);
                        } elseif (!BT_GmcpGsaDao::updateGsaOrders('order_status', 'partial returned', (string) $sGsaOrderId)) {
                            ...
                            if (is_array($aProductLinesToReturn) && isset($aProductLinesToReturn)) {
                                ...
                                foreach ($aProductLinesToReturn as $aProductData) {
                                    if (!BT_GmcpGsaDao::updateGsaOrdersProducts('quantity_returned', (int) $aProductData['product_quantity_return'], (string) $sGsaOrderId, (int) $aProductData['product_id'],  (int) $aProductData['product_attribute_id'])) {
