...
class CustomerFilters
{
    ...
    {
        $f = $this->prepareGroup($key) + ['type' => 0, 'forced_values' => []];
        if ($key == 'c') {
            $f['nesting_lvl'] = 0;
        }
        ...
        if ($this->isReady() && $cf = $this->get()) {
            ...
            foreach ($cf as $key => $id) {
                ...
                if (isset($f['values'][$id])) {
                    ...
                    if (empty($f['values'][$id]['selected'])) { // it could be selected in prepareFiltersData()
                        $params['filters'][$f['first_char']][$f['id_group']][] = $id;
                    }
                }
            }
