...
class AmazzingFilter extends Module
{
    ...
    public function install()
    {
        if (Shop::isFeatureActive()) {
            ...
            $this->x['shop_ids']['context'] = Shop::getContextListShopID();
        }
    ...
    public function oos($action, $params = [])
    {
        ...
        switch ($action) {
            ...
            case 'stockShopAssociation':
                $cache_id = 'shop_stock_' . $params['id_shop_group'] . '_' . $params['id_shop'];
                if (!$ret = $this->cache('get', $cache_id)) {
                    $share_stock = $this->db->getValue('
                        SELECT share_stock FROM ' . _DB_PREFIX_ . 'shop_group
                        WHERE id_shop_group = ' . (int) $params['id_shop_group'] . '
                    ');
                    $ret = $share_stock ? 'sa.id_shop_group = ' . (int) $params['id_shop_group']
    ...
    public function prepareInitialParams(&$filters)
    {
        ...
        foreach ($filters as $key => &$f) {
            ...
            foreach ($f['values'] as $id => $v) {
                if ($f['values'][$id]['selected'] = in_array($v['link'], $submitted_data)) {
                    $f['has_selection'] = 1;
    ...
    public function getCacheIDForFilterValues($f)
    {
        if ($f['first_char'] == 'c') {
            ...
            $caching_params = array_merge($caching_params, $this->customerGroupIDs());
        } else {
