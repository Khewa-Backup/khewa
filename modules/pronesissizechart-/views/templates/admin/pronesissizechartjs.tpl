...
{/literal}
	        {foreach from=$languages item=language}
	        	...
	        	$newCol += '<div class="row psc_lang{$language.id_lang|intval}" style="display:'+ style +'"><div class="table-flag"><img src="{$base_url}img/l/{$language.id_lang|intval}.jpg"></div><div class="col-xs-11"><input type="text" name="cell['+nval+']['+cval+'][{$language.id_lang|intval}]" id="r'+nval+'-c'+cval+'-l{$language.id_lang|intval}"></div></div>';
	        {/foreach}
{literal}
	        $newCol+= '</td>';
	        $("table.table-editor tbody tr:eq(" + n + ")").append($newCol);
	    });
	    $("#remove-cols").append("<td class='remove'><a class='btn btn-danger'><i class='icon-trash'></i></a></td>");
		});
		...
		$(document).on('click', '#addrow', function() {
		    ...
		    $('table.table-editor tr:eq(0) td').each(function(index, value){
		    		if(index==0)
		    		{
		        	$("table.table-editor tr:eq(-1)").append('<td class="remrow" width="3%"><a class="btn btn-danger"><i class="icon-trash"></i></a></td>');
		        } 
		        else 
		        {
...
{/literal}
		        	{foreach from=$languages item=language}
		        		style = ($('.psc_button[rel="psc_lang{$language.id_lang|intval}"]').hasClass('btn-success')) ? 'block' : 'none';
								$newRow += '<div class="row psc_lang{$language.id_lang|intval}" style="display:'+ style +'"><div class="table-flag"><img src="{$base_url}img/l/{$language.id_lang|intval}.jpg"></div><div class="col-xs-11"><input type="text" name="cell['+addedrow+']['+(index-1)+'][{$language.id_lang|intval}]" id="r'+addedrow+'-c'+(index-1)+'-l{$language.id_lang|intval}"></div></div>';
		        	{/foreach}
{literal}
		        	$newRow += '</td>';
