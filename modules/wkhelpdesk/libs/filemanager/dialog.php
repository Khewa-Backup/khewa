...
//view type
    ...
    /* PrestaShop */
        if (isset($src_thumb) && $src_thumb) {
            if (($src_thumb = preg_replace('#('.addslashes($current_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'modules/wkhelpdesk/libs/source/'), $src_thumb)) == $src_thumb) {
                $src_thumb = preg_replace('#('.addslashes($thumbs_base_path).')#ism', Tools::safeOutput(Context::getContext()->shop->physical_uri.'modules/wkhelpdesk/libs/source/'), $src_thumb);
            }
        }
        if (isset($mini_src) && $mini_src) {
            if (($mini_src = preg_replace('#('.addslashes($current_path).')#ism', Tools::safeOutput($upload_dir), $mini_src)) == $mini_src) {
                $mini_src = preg_replace('#('.addslashes($thumbs_base_path).')#ism', Tools::safeOutput($thumbs_base_path), $mini_src);
            }
        }
    /* END PrestaShop */
