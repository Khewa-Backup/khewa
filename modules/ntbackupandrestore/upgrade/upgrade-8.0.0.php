    ...
    foreach ($shops as $shop) {
        ...
        if (extension_loaded('mcrypt') && extension_loaded('openssl') && function_exists('hash_equals')) {
            ...
            if (extension_loaded('openssl') && function_exists('hash_equals')) {
                $iv_size = openssl_cipher_iv_length(Apparatus::CIPHER_CRYPTAGE);
                $iv = openssl_random_pseudo_bytes($iv_size);

                $encrypted_string = openssl_encrypt(
                    $owncloud_old_pass,
                    Apparatus::CIPHER_CRYPTAGE,
                    Apparatus::CLE_CRYPTAGE,
                    OPENSSL_RAW_DATA,
                    $iv
                );

                if ($encrypted_string !== false) {
                    $hmac = Apparatus::hash_hm('sha256', $encrypted_string, Apparatus::CLE_CRYPTAGE, true);
