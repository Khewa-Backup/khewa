		...
		<table class="table tableDnD" id="imageTable">
			<thead>
				<tr class="nodrag nodrop">
					...
					{if $shops}
						{foreach from=$shops item=shop}
							<th class="fixed-width-xs"><span class="title_box">{$shop.name|escape:'html':'UTF-8'}</span></th>
						{/foreach}
					{/if}
		...
		<table id="lineType" style="display:none;">
			<tr id="image_id" data-ids="images_id">
				...
				{if $shops}
					{foreach from=$shops item=shop}
						<td>
							<input type="checkbox" class="image_shop" name="imageShop" id="{$shop.id_shop|intval}image_id"
...
<div class="panel">
	...
	<div class="form-group">
		<label class="control-label col-lg-3">
			<span class="label-tooltip" data-toggle="gc_tooltip"
				title="{l s='Allow customers to personalize their "Print at home" gift cards with a beneficiary name and message.' mod='thegiftcard'}">
				{l s='Show customization fields on "Print at home" gift cards' mod='thegiftcard'}
			</span>
		</label>
		<div class="col-lg-9">
			<span class="switch prestashop-switch fixed-width-lg">
				<input type="radio" name="use_customization" id="use_customization_on" value="1"
					{if Configuration::get('GIFTCARD_USE_CUSTOMIZATION')|intval}checked="checked" {/if} />
				<label class="t" for="use_customization_on">{l s='Yes' mod='thegiftcard'}</label>
				<input type="radio" name="use_customization" id="use_customization_off" value="0"
					{if !Configuration::get('GIFTCARD_USE_CUSTOMIZATION')|intval}checked="checked" {/if} />
				<label class="t" for="use_customization_off">{l s='No' mod='thegiftcard'}</label>
				<a class="slide-button btn"></a>
			</span>
		</div>
	</div>
...
<div class="panel">
	<div class="panel-heading">
		<i class="icon-pencil-square-o"></i> {l s='Email settings' mod='thegiftcard'}
	</div>
	<div class="form-group">
		<label class="control-label col-lg-3">
			<span class="label-tooltip" data-toggle="gc_tooltip">
	...
	<div class="form-group">
		<label class="control-label col-lg-3">
			<span class="label-tooltip" data-toggle="gc_tooltip"
				title="{l s='Manage email templates' mod='thegiftcard'}">
				{l s='Email templates' mod='thegiftcard'}
			</span>
		</label>
		<div class="col-lg-9">
			<ul class="nav nav-pills">
				<li class="active">
					<a href="#buyer_email" data-toggle="gc_tab">{l s='Email "PRINT AT HOME"' mod='thegiftcard'}</a>
				</li>
				<li>
					<a href="#friend_email" data-toggle="gc_tab">{l s='Email "SEND TO A FRIEND"' mod='thegiftcard'}</a>
				</li>
			</ul>
			<hr>
								...
								{if $languages|count > 1}
									<div class="translatable-field row lang-{$language.id_lang|intval}">
										...
										<a class="btn btn-default" target="_blank"
											href="{$module->getAdminLink('AdminTranslations', ['type' => 'mails', 'selected-emails' => 'body', 'lang' => $language.iso_code|escape:'html':'UTF-8', 'selected-theme' => Context::getContext()->shop->theme_name|escape:'html':'UTF-8'])}#thegiftcard-giftcard_print">
											{l s='Edit email template' mod='thegiftcard'} </a>
										{if $languages|count > 1}
										</div>
										<div class="col-lg-2">
											<button type="button" class="btn btn-default dropdown-toggle"
							...
							{foreach from=$languages item=language}
								{if $languages|count > 1}
									<div class="translatable-field row lang-{$language.id_lang|intval}">
										...
										{/if}
										<a class="btn btn-default" target="_blank"
											href="{$module->getAdminLink('AdminTranslations', ['type' => 'mails', 'selected-emails' => 'body', 'lang' => $language.iso_code|escape:'html':'UTF-8', 'selected-theme' => Context::getContext()->shop->theme_name|escape:'html':'UTF-8'])}#thegiftcard-giftcard_friend">
											{l s='Edit email template' mod='thegiftcard'} </a>
										{if $languages|count > 1}
										</div>
										<div class="col-lg-2">
