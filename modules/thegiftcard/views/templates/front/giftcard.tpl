				...
				{if isset($category) && $category->id}
					<div class="row content_scene_cat">
						...
						<div class="content_scene_cat_bg"
							{if $category->id_image}style="background:url({$link->getCatImageLink($category->link_rewrite, $category->id_image)|escape:'quotes':'UTF-8'}) 0 bottom no-repeat; min-height:{$categorySize.height|intval}px;"
							{/if}>
					...
					<div class="row">
						<div class="col-lg-12">
							<div class="content">
								...
								{if $giftcard->text_fields|intval}
									<div id="card_text_fields" {if Configuration::get("GIFTCARD_USE_CUSTOMIZATION") == 0}style="display:none;"{/if}>
										{counter start=0 assign='customizationField'}
										{foreach from=$customizationFields item='field' name='customizationFields'}
											<div class="form-group {if in_array($field.id_customization_field, [Configuration::get("GIFTCARD_CUST_DATE_`$giftcard->id|intval`"), Configuration::get("GIFTCARD_CUST_EMAIL_`$giftcard->id|intval`")])}hide show-for-send-to-friend{/if}">
												<div class="form-label">
