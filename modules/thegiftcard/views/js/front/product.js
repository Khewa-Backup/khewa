...
function initBinds() {
	...
	$('#block_customization input[name="sending_method"]').on('change', function () {
		...
		if (sendingMethod == printAtHome) {
			if (useGiftCardCustomization) {
				$('#card_text_fields .form-group').filter('.show-for-send-to-friend').addClass('hide');
			} else {
				$('#card_text_fields').hide();
			}
		} else {
			$('#card_text_fields .form-group').removeClass('hide');
...
function ajaxCall(action) {
	...
	if (useGiftCardCustomization || sendingMethod == sendToFriend) {
		$('#card_text_fields .form-group').not('.hide').find('input[type=text], textarea').each(function () {
