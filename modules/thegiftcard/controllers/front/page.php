...
class ThegiftcardPageModuleFrontController extends ModuleFrontController
{
    ...
    public function display()
    {
        ...
        Media::addJsDef(['useGiftCardCustomization' => (int) Configuration::get('GIFTCARD_USE_CUSTOMIZATION')]);
    ...
    public function initContent()
    {
        ...
        if (Validate::isLoadedObject($this->product) && $this->product->isAssociatedToShop()) {
            ...
            $customAmountFrom = (int) Configuration::get('GIFTCARD_AMOUNT_CUSTOM_FROM_' . (int) $this->product->id);
            $customAmountTo = (int) Configuration::get('GIFTCARD_AMOUNT_CUSTOM_TO_' . (int) $this->product->id);

            $this->context->smarty->assign([
                'giftcard' => $this->product,
                'active' => Configuration::get('GIFTCARD_ACTIVE_' . (int) $this->product->id),
                'pitch' => $pitch,
                'pitch_formatted' => $this->module->displayPrice((float) $pitch, (int) $this->context->currency->id),
                'custom_amount_from' => $customAmountFrom,
                'custom_amount_from_formatted' => $this->module->displayPrice((float) $customAmountFrom, (int) $this->context->currency->id),
                'custom_amount_to' => $customAmountTo,
                'custom_amount_to_formatted' => $this->module->displayPrice((float) $customAmountTo, (int) $this->context->currency->id),
    ...
    protected function assignAttributesGroups()
    {
        ...
        if (Validate::isLoadedObject($template_group_object)) {
            ...
            foreach ($template_attributes as $template_attribute) {
                ...
                $image = [];
                $image['thumbnail'] = $this->module->getThumbnail($this->product, $image_obj);
            ...
            if (count($images)) {
                ...
                $template_vars = [
                    ...
                    'rewrite_group_name' => str_replace(Configuration::get('PS_ATTRIBUTE_ANCHOR_SEPARATOR'), '_', Tools::str2url(str_replace([',', '.'], '-', $template_group_object->public_name[(int) $this->context->language->id]))),
                    'attributes' => $images,
                    'tags' => $tags,
                ];
            }
        ...
        if (Validate::isLoadedObject($amount_group_object)) {
            ...
            foreach ($amount_attributes as $amount_attribute) {
                ...
                $amount['attribute_value_formatted'] = $this->module->displayPrice((float) $amount_attribute['name'], $this->context->currency->id);
    ...
    public function displayAjaxGetCombination()
    {
        ...
        if ((GiftCardModel::SEND_TO_FRIEND == $sending_method || Configuration::get('GIFTCARD_USE_CUSTOMIZATION'))
            && !is_array(Tools::getValue('customizationData'))
        ) {
        ...
        if (!($template = GiftCardModel::filterAttribute(
            ...
            GiftCardModel::getImagesList($this->product->id, $this->context->language->id)
        ))) {
        ...
        if (!($amount = GiftCardModel::filterAttribute(
            ...
            (int) $this->context->language->id,
            GiftCardModel::getAmountsList($this->product->id)
        ))) {
            exit(json_encode(['error' => $this->module->l('Please select an amount', 'page')]));
        }

        if (!($id_combination = $this->module->generateCombination($this->product->id, [$template['id_attribute'], $amount['id_attribute']]))) {
    ...
    public function displayAjaxGeneratePdf()
    {
        ...
        $pdf_vars = [
            ...
            'giftcard_amount' => $this->module->displayPrice((float) $amount['name'], (int) Context::getContext()->currency->id),
