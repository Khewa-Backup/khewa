...
$html_description = '<div class="row" style="padding-top: 5px;"><div class="col-md-12">
<label>'.$langtext[0].'</label></div></div>';
foreach ($languages as $lang)
	$html_description .= '<div class="row"><div class="col-md-12"><img src="'.
											__PS_BASE_URI__.'img/l/'.$lang['id_lang'].
											'.jpg" style="padding-top:8px"></div><div class="col-md-12"><input type="text" class="form-control" name="description['.
											$lang['id_lang'].']" value="'.htmlentities($description[$lang['id_lang']], ENT_QUOTES, 'UTF-8').'"></div></div>';

$buttons = '';
foreach ($languages as $lang)
	$buttons .= '<a class="psc_button btn" rel="psc_lang'.$lang['id_lang'].'"><i class="icon-times"></i> '.
...
{
	if ($firstcol <= 1)
		$html_content .= '<td><a style="opacity:0" class="btn"><i class="material-icons">delete</i></a></td>';
	else
		$html_content .= '<td class="remove"><a class="btn"><i class="material-icons">delete</i></a></td>';
	$firstcol++;
}
...
foreach ($value as $rows => $row)
{
	...
	if ($firstrow == 0)
		...
		$html_content .= '<td class="remrow" width="3%"><a class="btn"><i class="material-icons">delete</i></a></td>';
	foreach ($row as $cols => $col)
	{
		...
		foreach ($languages as $lang)
			$html_content .= '<div class="row psc_lang'.$lang['id_lang'].'" style="display:block"><div class="table-flag"><img src="'.__PS_BASE_URI__.
											'img/l/'.$lang['id_lang'].'.jpg"></div><div class="col-md-12"><input value="'.htmlentities($col[$lang['id_lang']], ENT_QUOTES, 'UTF-8').
												'" type="text" class="form-control" name="cell['.$rows.']['.$cols.']['.$lang['id_lang'].']" id="r'.
