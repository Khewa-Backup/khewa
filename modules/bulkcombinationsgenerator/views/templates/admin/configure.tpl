...
{if !empty($bcg_js_vars)}
<script type="text/javascript">
{foreach $bcg_js_vars as $name => $value}
...
<div class="bootstrap bcg-container clearfix">
	<div class="att-actions clearfix">
		...
		<div class="im-export pull-right">
			...
			<form action="" method="POST" class="inline-block">
				<a href="#" class="no-decoration restoreSettings uppercase"
				data-toggle="tooltip" title="{l s='Restore last' mod='bulkcombinationsgenerator'}">
					<i class="icon-download icon-repeat"></i> {l s='Restore' mod='bulkcombinationsgenerator'}
				</a>
			</form>
			*}
			<form action="" method="POST" class="inline-block">
	...
	<div class="panel attributes cleafix">
		<div id="manual-assign" class="tab-content active clearfix">
			<form method="post" action="" class="form-horizontal attributes-form clearfix">
				<button type="button" class="btn btn-primary showAttributes" data-toggle="modal" data-target="#bcg-dynamic-popup">
						...
						<tr>
							...
							{foreach $combination_fields as $key => $field}
								<th class="text-center">
									...
									{if $key == 'price'}
										<div class="inline-block"><select name="a[options][tax_incl]" class="minimalistic tax-incl">
				...
				<div class="selected-att-options clearfix">
	...
	<div class="row">
		<div class="col-lg-12 override-settings hidden">
			<div class="panel">
				<div class="panel-header">
					<i class="icon-refresh"></i> {l s='Specify fields that should be updated for [1]existing[/1] combinations' mod='bulkcombinationsgenerator' tags=['<span class="u">']}
				</div>
				<form action="" method="POST" class="override-settings-form">
					{foreach $attribute_options_fields as $name => $field}
						...
						{/if}
					{/foreach}
					<label class="override-label for-impacts hidden">
						<input name="a[override_options][impacts]" type="checkbox" value="1" class="toggleOverrideImpactOptions">
						{l s='Price/Weight impacts' mod='bulkcombinationsgenerator'}
					</label>
					<div class="inline-block override-impacts hidden">
						<select name="a[override_options][erase_impacts]">
							<option value="">{l s='Add new values to existing impacts' mod='bulkcombinationsgenerator'}</option>
							<option value="1">{l s='Erase previous impacts' mod='bulkcombinationsgenerator'}</option>
						</select>
					</div>
				</form>
			</div>
		...
		<div class="col-lg-6">
			<div class="panel clearfix">
				...
				{foreach $product_filters as $name => $filter}
					...
					<div class="form-group">
						<label class="filter-label control-label col-lg-3">{$filter.label|escape:'html':'UTF-8'}</label>
						<div class="filter-value col-lg-9">
					...
					<div class="form-group">
						<div class="combination-actions col-lg-10">
							<select name="action" class="processAction">
								<option value="update">{l s='Update combinations (add new if required)' mod='bulkcombinationsgenerator'}</option>
								<option value="updateByAtts">{l s='Update parameters of existing combinations (don\'t add new)' mod='bulkcombinationsgenerator'}</option>
								<option value="addNew">{l s='Add new combinations (don\'t touch existing)' mod='bulkcombinationsgenerator'}</option>
								<option value="regenerate">{l s='Re-Generate all combinations' mod='bulkcombinationsgenerator'}</option>
								<option value="deleteByAtts">{l s='Delete combinations with selected attributes' mod='bulkcombinationsgenerator'}</option>
								<option value="delete">{l s='Delete all existing combinations' mod='bulkcombinationsgenerator'}</option>
								<option value="duplicate" class="hidden">{l s='Duplicate combinations' mod='bulkcombinationsgenerator'}</option>
							</select>
						</div>
...
<div class="modal fade" id="bcg-dynamic-popup" tabindex="-1">
