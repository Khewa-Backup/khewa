...
namespace ETSHybridauth\Thirdparty\OpenID;

use ETSHybridauth\Exception\Exception;
use ETSHybridauth\Exception\ExceptionInterface;
...
class LightOpenID
{
     ...
     * @return bool, if the server exists; false otherwise
     */
    ...
    protected function request_streams($url, $method = 'GET', $params = array(), $update_claimed_id = null)
    {
        ...
        if (!empty($data)) {
            $http_response_header = json_decode($data);
    ...
    protected function request($url, $method = 'GET', $params = array(), $update_claimed_id = false)
    {
        ...
        if (function_exists('curl_init')) {
            ...
            if (!$use_curl && !in_array('https', stream_get_wrappers())) {
                $use_curl = true;
            }

            if (!$use_curl && (ini_get('safe_mode') || ini_get('open_basedir'))) {
    ...
    public function discover($url)
    {
        ...
        # Initialize content variable to avoid undefined variable warnings
        $content = null;
        ...
        for ($i = 0; $i < 5; $i++) {
            if ($yadis) {
                ...
                if (isset($headers['content-type']) && $this->is_allowed_type($headers['content-type'])) {
                    ...
                    # If we reach here, no valid OpenID service was found in XRDS
                    # Switch to HTML discovery
                    $yadis = false;
                    $url = $originalUrl;
                    $content = null;
                    continue; # Go to next iteration to try HTML discovery
                }
    ...
    public function validate()
    {
        ...
        foreach (explode(',', $this->data['openid_signed']) as $item) {
            ...
            // Since magic quotes were always disabled by default in newer PHP versions, 
            // we don't need to check for it anymore
