...
class WpFooterSlide extends ObjectModel
{
    public $title;
    public $url;
    public $image;
    ...
    public static $definition = array(
        'table' => 'wpfooterslider_slides',
        'primary' => 'id_wpfooterslider_slides',
        'multilang' => true,
        'fields' => array(
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $res &= Db::getInstance()->execute('
            INSERT INTO `'._DB_PREFIX_.'wpfooterslider` (`id_shop`, `id_wpfooterslider_slides`)
    ...
    public function delete()
    {
        ...
        $res &= Db::getInstance()->execute('
            DELETE FROM `'._DB_PREFIX_.'wpfooterslider`
            WHERE `id_wpfooterslider_slides` = '.(int)$this->id);
    ...
    public function reOrderPositions()
    {
        ...
        $max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            WHERE hss.`id_wpfooterslider_slides` = hs.`id_wpfooterslider_slides` AND hs.`id_shop` = '.(int)$id_shop);
        ...
        $rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hss.`position` as position, hss.`id_wpfooterslider_slides` as id_slide
            FROM `'._DB_PREFIX_.'wpfooterslider_slides` hss
            LEFT JOIN `'._DB_PREFIX_.'wpfooterslider` hs ON (hss.`id_wpfooterslider_slides` = hs.`id_wpfooterslider_slides`)
            WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position);

        foreach ($rows as $row) {
            $current_slide = new WpFooterSlide($row['id_slide']);
