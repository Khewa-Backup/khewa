...
class LoyaltyModule extends ObjectModel
{
    ...
    public static function getCartNbPoints(
        $cart,
        $newProduct = null
    ) {
        $total = 0;
    ...
    public static function getVoucherValue(
        $nbPoints,
        $id_currency = null
    ) {
    ...
    public static function getAllByIdCustomer(
        $id_customer,
        $id_lang,
        $onlyValidate = false,
        $pagination = false,
    ...
    public static function getDiscountByIdCustomer(
        $id_customer,
        $last = false
    ) {
        $query = '
    ...
    public static function registerDiscount($cartRule)
    {
        ...
        foreach ($items as $item) {
            ...
            $negativePoints = (int)Db::getInstance()
                                     ->getValue('SELECT SUM(points) points FROM '._DB_PREFIX_.'loyalty WHERE id_order = '.(int)$item['id'].' AND id_loyalty_state = '.(int)LoyaltyStateModule::getCancelId().' AND points < 0');
