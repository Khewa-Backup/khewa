...
class LoyaltypointsrsiDefaultModuleFrontController extends ModuleFrontController
{
    public $ssl = true;
    ...
    public function __construct()
    {
        ...
        smartyRegisterFunction($this->context->smarty, 'function', 'summarypaginationlink', array('LoyaltypointsrsiDefaultModuleFrontController', 'getSummaryPaginationLink'));
    }
    ...
    public function processTransformPoints()
    {
        ...
        if ($customer_points > 0) {
            ...
            if ($contains_categories) {
                ...
                Db::getInstance()
                  ->execute($sql);
                $id_product_rule = (int)Db::getInstance()
                                          ->Insert_ID();
        ...
        Tools::redirect($this->context->link->getModuleLink('loyaltypointsrsi', 'default', array('process' => 'summary')));
    }
    /**
     ...
     * @return string link
     */
    public static function getSummaryPaginationLink(
        $params,
        &$smarty
    ) {
        if (!isset($params['p'])) {
    ...
    public function assignSummaryExecution()
    {
        ...
        $this->context->smarty->assign(
            array('validlp' => Configuration::get('PS_LOYALTY_VALIDITY_PERIOD'),'orders' => $orders, 'displayorders' => $displayorders, 'totalPoints' => (int)$customer_points, 'voucher' => LoyaltyModule::getVoucherValue($customer_points, (int)$this->context->currency->id), 'validation_id' => LoyaltyStateModule::getValidationId(), 'transformation_allowed' => $customer_points > 0, 'pagen' => ((int)Tools::getValue('p') > 0 ? (int)Tools::getValue('p') : 1), 'nbpagination' => ((int)Tools::getValue('n') > 0 ? (int)Tools::getValue('n') : 10), 'nArray' => array(10, 20, 50), 'max_page' => floor(count($orders) / ((int)Tools::getValue('n') > 0 ? (int)Tools::getValue('n') : 10)), 'pagination_link' => Context::getContext()->link->getModuleLink('loyalty', 'default'))
        );
        /* Discounts */
        $nb_discounts = 0;
        $discounts = array();
        ...
        $this->context->smarty->assign(
            array('nbDiscounts' => (int)$nb_discounts, 'discounts' => $discounts, 'minimalLoyalty' => (float)Configuration::get('PS_LOYALTY_MINIMAL'), 'categories' => $categories_names)
        );
        //$this->setTemplate('loyalty.tpl');
        $this->setTemplate('module:loyaltypointsrsi/views/templates/front/loyalty.tpl');
    }
}
