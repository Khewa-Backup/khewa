...
class Loyaltypointsrsi extends Module
{
    protected $html = '';
    public function __construct()
    {
        $this->name = 'loyaltypointsrsi';
        $this->tab = 'pricing_promotion';
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);
        $this->module_key = '75c54e49f9d7cda8ea94caecc2827d84';
        $this->version = '2.1.0';
        $this->author = 'RSI';
        ...
        $this->description = $this->l('Provide a loyalty program to your customers for PS 1.7.');
    ...
    {
        include_once(dirname(__FILE__).'/LoyaltyStateModule.php');
        if (!parent::install() || !$this->installDB() || !$this->registerHook('displayProductAdditionalInfo') || !$this->registerHook('updateOrderStatus') || !$this->registerHook('newOrder') || !$this->registerHook('adminCustomers') || !$this->registerHook('shoppingCart') || !$this->registerHook('orderReturn') || !$this->registerHook('cancelProduct') || !$this->registerHook('customerAccount') || !Configuration::updateValue('PS_LOYALTY_POINT_VALUE', '0.20') || !Configuration::updateValue('PS_LOYALTY_MINIMAL', 0) || !Configuration::updateValue('PS_LOYALTY_POINT_RATE', '10') || !Configuration::updateValue('PS_LOYALTY_NONE_AWARD', '1') || !Configuration::updateValue('PS_LOYALTY_TAX', '0') || !Configuration::updateValue('PS_LOYALTY_VALIDITY_PERIOD', '0')) {
            return false;
        }
    ...
    {
        if (!parent::uninstall() || !$this->uninstallDB() || !Configuration::deleteByName('PS_LOYALTY_POINT_VALUE') || !Configuration::deleteByName('PS_LOYALTY_POINT_RATE') || !Configuration::deleteByName('PS_LOYALTY_NONE_AWARD') || !Configuration::deleteByName('PS_LOYALTY_MINIMAL') || !Configuration::deleteByName('PS_LOYALTY_VOUCHER_CATEGORY') || !Configuration::deleteByName('PS_LOYALTY_VOUCHER_DETAILS') || !Configuration::deleteByName('PS_LOYALTY_TAX') || !Configuration::deleteByName('PS_LOYALTY_VALIDITY_PERIOD')) {
            return false;
        }
        return true;
    }
    ...
    {
        Db::getInstance()
          ->execute('DROP TABLE `'._DB_PREFIX_.'loyalty`;');
        Db::getInstance()
          ->execute('DROP TABLE `'._DB_PREFIX_.'loyalty_state`;');
        Db::getInstance()
          ->execute('DROP TABLE `'._DB_PREFIX_.'loyalty_state_lang`;');
        Db::getInstance()
          ->execute('DROP TABLE `'._DB_PREFIX_.'loyalty_history`;');
        return true;
    }
    private function _postProcess()
    {
        if (Tools::isSubmit('submitLoyalty')) {
            ...
            if (!count($this->_errors)) {
                ...
                if (!Validate::isFloat(Tools::getValue('point_value'))) {
                    $errors[] = $this->l('Invalid value for point value');
                } else {
                    Configuration::updateValue('PS_LOYALTY_POINT_VALUE', (float)(Tools::getValue('point_value')));
                }
                if (!Validate::isInt(Tools::getValue('point_rate'))) {
                    $errors[] = $this->l('Invalid value for point rate');
                } else {
                ...
                if (!Validate::isInt(Tools::getValue('minimal'))) {
                    $errors[] = $this->l('Invalid value minimal');
                } else {
                ...
                if (!Validate::isInt(Tools::getValue('validity_period'))) {
                    $errors[] = $this->l('Invalid value for period');
                } else {
                    Configuration::updateValue('PS_LOYALTY_VALIDITY_PERIOD', (float)(Tools::getValue('validity_period')));
                }
    ...
    public function getContent()
    {
        ...
        $this->_postProcess();
        $this->html .= $this->_displayInfo().$this->renderForm();
    ...
    public function hookdisplayProductAdditionalInfo($params)
    {
        ...
        if (Validate::isLoadedObject($product)) {
            ...
            $this->smarty->assign(
                array('points' => (int)$points, 'total_points' => (int)$pointsAfter, 'point_rate' => Configuration::get('PS_LOYALTY_POINT_RATE'), 'point_value' => Configuration::get('PS_LOYALTY_POINT_VALUE'), 'points_in_cart' => (int)$pointsBefore, 'voucher' => LoyaltyModule::getVoucherValue((int)$pointsAfter), 'none_award' => Configuration::get('PS_LOYALTY_NONE_AWARD'))
            );
    ...
    {
        return $this->display(__FILE__, 'my-account2.tpl');
    }
    ...
    public function hookShoppingCart($params)
    {
        ...
        $this->smarty->assign(
            array('points' => (int)$points,'point_rate' => Configuration::get('PS_LOYALTY_POINT_RATE'), 'voucher' => LoyaltyModule::getVoucherValue((int)$points), 'guest_checkout' => (int)Configuration::get('PS_GUEST_CHECKOUT_ENABLED'))
        );
    ...
    public function hookAdminCustomers($params)
    {
        ...
        if (Tools::getValue('typelp') && Tools::getValue('pointslp') && Tools::getValue('pointslp') != null) {
            Db::getInstance()
                    ->Execute('INSERT INTO `' . _DB_PREFIX_ . 'loyalty` (`id_loyalty`, `id_loyalty_state`, `id_customer`, `id_order`, `id_cart_rule`, `points`, `date_add`, `date_upd`) VALUES (NULL, \''.pSQL(Tools::getValue('typelp')).'\', \''.(int)$params['id_customer'].'\', 0, 0 , \''.pSQL(Tools::getValue('pointslp')).'\', NOW(), NOW());');
            $id = Db::getInstance()->Insert_ID();        
            Db::getInstance()
                    ->Execute('INSERT INTO `' . _DB_PREFIX_ . 'loyalty_history` (`id_loyalty_history`, `id_loyalty`, `id_loyalty_state`, `points`, `date_add`) VALUES (NULL,\''.pSQL($id).'\', \''.pSQL(Tools::getValue('typelp')).'\', \''.pSQL(Tools::getValue('pointslp')).'\', NOW());');
        }
        ...
        $this->context->smarty->assign(
            array(
                'linkgen' => $protocol.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'].'&id_customer=' . (int)$params['id_customer'] . '&fc=bo',
                'token' => Tools::getAdminToken('AdminOrders'.(int)Tab::getIdFromClassName('AdminOrders').(int)$params['cookie']->id_employee),
                'points' => $points,
                'voucherlo' => Tools::displayPrice(LoyaltyModule::getVoucherValue((int)$points, (int)Configuration::get('PS_CURRENCY_DEFAULT')),new Currency((int)Configuration::get('PS_CURRENCY_DEFAULT'))),
                'details' => $details,
            )
        );
        return $this->display(
            __FILE__,
            'views/templates/admin/loy.tpl'
        );
    }
    ...
    public function ajax($pricelp, $productid)
    {
        $product = new Product((int)$productid);
        if (Validate::isLoadedObject($product)) {
            if (Validate::isLoadedObject(Context::getContext()->cart)) {
                $pointsBefore = (int)LoyaltyModule::getCartNbPoints(Context::getContext()->cart);
                $pointsAfter = (int)LoyaltyModule::getCartNbPoints(Context::getContext()->cart, $product);
                $points = (int)($pointsAfter - $pointsBefore);
            } else {
                if (!(int)Configuration::get('PS_LOYALTY_NONE_AWARD') && Product::isDiscounted((int)$product->id)) {
                    $points = 0;
            ...
            return array( 'points' => (int)$points,
    ...
    {
        $fields_values = array('point_rate' => Tools::getValue('PS_LOYALTY_POINT_RATE', Configuration::get('PS_LOYALTY_POINT_RATE')), 'point_value' => Tools::getValue('PS_LOYALTY_POINT_VALUE', Configuration::get('PS_LOYALTY_POINT_VALUE')), 'PS_LOYALTY_NONE_AWARD' => Tools::getValue('PS_LOYALTY_NONE_AWARD', Configuration::get('PS_LOYALTY_NONE_AWARD')), 'minimal' => Tools::getValue('PS_LOYALTY_MINIMAL', Configuration::get('PS_LOYALTY_MINIMAL')), 'validity_period' => Tools::getValue('PS_LOYALTY_VALIDITY_PERIOD', Configuration::get('PS_LOYALTY_VALIDITY_PERIOD')), 'id_order_state_validation' => Tools::getValue('id_order_state_validation', $this->loyaltyStateValidation->id_order_state), 'id_order_state_cancel' => Tools::getValue('id_order_state_cancel', $this->loyaltyStateCancel->id_order_state), 'PS_LOYALTY_TAX' => Tools::getValue('PS_LOYALTY_TAX', Configuration::get('PS_LOYALTY_TAX')),);
