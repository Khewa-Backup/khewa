...
include_once(dirname(__FILE__).'/loyaltypointsrsi.php');
Tools::displayFileAsDeprecated();
$context = Context::getContext();
if (!$context->customer->isLogged()) {
    Tools::redirect('index.php?controller=authentication&back=modules/loyaltypointsrsi/loyalty-program.php');
}
$context->controller->addJqueryPlugin(array('dimensions', 'cluetip'));
$customerPoints = (int)LoyaltyModule::getPointsByCustomer((int)$context->cookie->customer->id);
...
if (Tools::getValue('transform-points') == 'true' and $customerPoints > 0) {
    ...
    $dateFrom = Db::getInstance()
                  ->getValue(
                      '
	SELECT UNIX_TIMESTAMP(date_add) n
	FROM '._DB_PREFIX_.'loyalty
	WHERE id_cart_rule = 0 AND id_customer = '.(int)$context->cookie->customer->id.'
    ...
    Tools::redirect('modules/loyaltypointsrsi/loyalty-program.php');
}
include(dirname(__FILE__).'/../../header.php');
$orders = LoyaltyModule::getAllByIdCustomer((int)($context->cookie->customer->id), (int)($context->cookie->language->id));
$displayorders = LoyaltyModule::getAllByIdCustomer((int)($context->cookie->customer->id), (int)($context->cookie->language->id), false, true, ((int)(Tools::getValue('n')) > 0 ? (int)(Tools::getValue('n')) : 10), ((int)(Tools::getValue('p')) > 0 ? (int)(Tools::getValue('p')) : 1));
$context->smarty->assign(
    array('orders' => $orders, 'displayorders' => $displayorders, 'pagination_link' => __PS_BASE_URI__.'modules/loyaltypointsrsi/loyalty-program.php', 'totalPoints' => (int)$customerPoints, 'voucher' => LoyaltyModule::getVoucherValue($customerPoints, (int)$context->currency->id), 'validation_id' => LoyaltyStateModule::getValidationId(), 'transformation_allowed' => $customerPoints > 0, 'page' => ((int)(Tools::getValue('p')) > 0 ? (int)(Tools::getValue('p')) : 1), 'nbpagination' => ((int)(Tools::getValue('n') > 0) ? (int)(Tools::getValue('n')) : 10), 'nArray' => array(10, 20, 50), 'max_page' => floor(sizeof($orders) / ((int)(Tools::getValue('n') > 0) ? (int)(Tools::getValue('n')) : 10)))
);
/* Discounts */
$nbDiscounts = 0;
$discounts = array();
if ($ids_discount = LoyaltyModule::getDiscountByIdCustomer((int)$context->cookie->customer->id)) {
    $nbDiscounts = count($ids_discount);
    foreach ($ids_discount as $key => $discount) {
        $discounts[$key] = new Discount((int)$discount['id_cart_rule'], (int)$context->cookie->language->id);
        $discounts[$key]->orders = LoyaltyModule::getOrdersByIdDiscount((int)$discount['id_cart_rule']);
    }
}
$allCategories = Category::getSimpleCategories((int)($context->cookie->language->id));
...
$context->smarty->assign(
    array('nbDiscounts' => (int)$nbDiscounts, 'discounts' => $discounts, 'minimalLoyalty' => (float)Configuration::get('PS_LOYALTY_MINIMAL'), 'categories' => $categoriesNames)
);
$loyalty = new Loyalty();
echo $loyalty->display(dirname(__FILE__).'/loyaltypointsrsi.php', 'loyalty.tpl');
