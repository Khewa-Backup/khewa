...
<br/>
<div class="card text-center" style="border: 1px solid #ccc;
border-radius: 9px;padding: 13px;">
  <div class="card-body">
    <h5 class="card-title">{l s='Points expiration' mod='loyaltypointsrsi'}</h5>
    <p class="card-text">{l s='Points last for'  mod='loyaltypointsrsi'} {$validlp} {l s='days' mod='loyaltypointsrsi'}. {l s='After that time, they expire and cannot be used.'  mod='loyaltypointsrsi'}</p>
  </div>
</div>
<br/>
{if $orders}
<div class="block-center" id="block-history">
	...
	<table id="order-list" class="table table-striped table-bordered table-labeled hidden-sm-down">
		...
		{foreach from=$displayorders item='order'}
			<tr class="alternate_item">
			{assign var="date" value= $order.date|@strtotime + (60*60*24*24)}
            {assign var="date2" value=$date|date_format:"%d/%m/%Y %H:%M:%S"}
...
<div class="bottom-pagination-content clearfix">
	...
	<div>
					<a class="btn btn-default button exclusive-medium" href="{url entity='module' name='loyaltypointsrsi' controller='default' params=['process' => 'summary', 'p' => 1, 'n' => 10000, 'fc' => 'module']}">
	...
	<div class="product-count">
		...
		{else}
		    {assign var='ordc' value=$orders|@count}
			{assign var='itemShowing' value=($nbpagination*$pagen-$ordc-$nbpagination*$pagen)*-1}
		{/if}
		{if $pagen==1}
