  ...
  class updateProductCatalog
  {
    ...
    public function update( $limit = false )
    {
      ...
      if( !$this->_limit ){
        ...
        Configuration::updateValue('UPDATED_PRODUCTS_OLD_IMAGES', updateProductTools::jsonEncode(array()), false, $this->_shopGroupId, $this->_idShop);
        ...
        $this->_updatedProducts = (int)Configuration::get('UPDATED_PRODUCTS_CURRENT_COUNT', null ,$this->_shopGroupId, $this->_idShop);
      }
      ...
      $images = updateProductTools::jsonDecode(Configuration::get('UPDATED_PRODUCTS_OLD_IMAGES', null ,$this->_shopGroupId, $this->_idShop));
      ...
      if( $this->_updatedProducts != $this->_productsForUpdate ){
        ...
        $res = array(
          ...
          'error_logs'  => Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.'modules/updateproducts/error/error_logs.csv',
        );
      ...
      if( Configuration::get('GOMAKOIL_IMPORT_IMAGES_ERROR_LOG', null, $this->_shopGroupId, $this->_idShop) ){
        $res['error_logs'] = Tools::getShopDomainSsl(true, true).__PS_BASE_URI__.'modules/updateproducts/error/error_logs.csv';
      }
    ...
    {
      $write_fd = fopen(_PS_MODULE_DIR_ . 'updateproducts/error/error_logs.csv', 'w');
      ...
      foreach ($this->_PHPExcelFactory->getWorksheetIterator() as $worksheet) {
        ...
        for ($row = 1; $row <= $rowLimit; ++ $row) {
          ...
          for ($col = 0; $col < $highestColumnIndex; ++ $col) {
            ...
            if( $val && is_numeric($val) && PHPExcel_Shared_Date::isDateTime($cell) ) {
              $dateValue = PHPExcel_Shared_Date::ExcelToPHPObject($val)->format('U');
              $val = date('Y-m-d H:i:s', $dateValue);
            }
          ...
          if($product){
            ...
            if (!$this->isProductExists($product['id_product'])) {
                $error_message = sprintf(Module::getInstanceByName('updateproducts')->l('Product with ID - %1s does not exists.'), $product['id_product']);
                $this->_createErrorsFile($error_message, $product['id_product']);
                continue;
            }
    ...
    private function _updateProductSeparate( $product )
    {
      ...
      foreach( $product as $field => $value ){
        ...
        if( $field == "categories_ids" ){
          ...
          $this->updateCategories($productObject, $value);
        }
        elseif( $field == "image_caption" &&  !isset($product['images']) ){
          ...
          if(isset($value) && $value){
            ...
            $id_product_attribute = (int)trim($product['id_product_attribute']);
        ...
        elseif($field == "pack_items_id"){
          $pack_items_id = $product['pack_items_id'];
          $pack_items_attribute_id = $product['pack_items_id_pack_product_attribute'];
          $pack_items_quantity = $product['pack_items_quantity'];
          if( $pack_items_id ){
            $pack_items_id = explode(';', $pack_items_id);
            $pack_items_attribute_id = explode(';', $pack_items_attribute_id);
            $pack_items_quantity = explode(';', $pack_items_quantity);

            $productType = $productObject->getType();
            if( $productType == Product::PTYPE_PACK || ($productType == Product::PTYPE_SIMPLE && !$productObject->getDefaultIdProductAttribute() ) ){
              $i = 0;
              foreach( $pack_items_id as $key => $pack_item_id ){
                if( $this->isProductExists($pack_item_id) ){
                  $tmpProduct = new Product((int)$pack_item_id, false);
                  $productType = $tmpProduct->getType();
                  if( $productType != Product::PTYPE_PACK && $productType != Product::PTYPE_VIRTUAL ){
                    if( $i == 0 ){
                      $productObject->deletePack();
                    }
                    $i++;
                    Pack::addItem((int)$productObject->id, (int)$pack_item_id, (int)$pack_items_quantity[$key], (int)$pack_items_attribute_id[$key]);
                  }
                }
              }
            }
        ...
        elseif( $field == "images" ){
          ...
          if(isset($value) && $value){
            ...
            $settings = updateProductTools::jsonDecode(Configuration::get('UPDATED_PRODUCTS_OLD_IMAGES', null ,$this->_shopGroupId, $this->_idShop));
            $all = array_merge($settings, $this->_old_images);
            Configuration::updateValue('UPDATED_PRODUCTS_OLD_IMAGES', updateProductTools::jsonEncode($all), false, $this->_shopGroupId, $this->_idShop);
        ...
        elseif (preg_match('/^attribute_group_(\d+)$/', $field, $attribute_group_matches)) {
            ...
            $id_attribute_group = $attribute_group_matches[1];
            $attribute_name = $value;
            $id_product_attribute = $product['id_product_attribute'];

            $this->updateAttribute($attribute_name, $id_attribute_group, $id_product_attribute);
        }
        elseif( $field == "combinations_price" ){
          $attribute =  (int)trim($product['id_product_attribute']);
          $values = trim($product['combinations_price']);
          if( $attribute && $values != "" ){
            $combination = new Combination((int)$attribute);
            $combination->price = $this->_preparePrice($values);
        ...
        elseif( $field == "combinations_price_with_tax" ){
          ...
          if( $attribute && $values != '' ){
            ...
            $combination->price = $this->_preparePrice($taxPrice);
            if( ( $error = $combination->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $combination->update();
        ...
        elseif( $field == "combinations_final_price" ){
          ...
          if (empty($product['id_product_attribute']) || empty($final_price)) {
              continue;
          }

          $combination_impact_on_price = $final_price - $current_product_price;

          $combination = new Combination((int)trim($product['id_product_attribute']));
          $combination->price = $this->_preparePrice($combination_impact_on_price);
          $combination->update();
        }
        ...
        elseif( $field == "combinations_wholesale_price" ){
          ...
          if( $attribute && $values != "" ){
            ...
            $combination->wholesale_price = $this->_preparePrice($values);
            if( ( $error = $combination->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $combination->update();
        ...
        elseif( $field == "combinations_unit_price_impact" ){
          ...
          if( $attribute && $values != "" ){
            $combination = new Combination($attribute);
            $combination->unit_price_impact = $this->_preparePrice($values);
            if( ( $error = $combination->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $combination->update();
        ...
        elseif( $field == "minimal_quantity" ){
          if( trim($product['id_product_attribute']) ){
            ...
            if( $values != "" && $attribute ){
              $combination = new Combination($attribute);
              $combination->minimal_quantity = (int)$values;
              if( ( $error = $combination->validateFields(false, true) ) !== true ){
                return $this->_createErrorsFile($error, $product['id_product']);
              }
              $combination->update();
          ...
          if( trim($product['id_product_attribute']) ){
            ...
            if( $attribute ){
              if(method_exists(new StockAvailable(), 'setLocation')){
                StockAvailable::setLocation((int)$product['id_product'], (string)$values, $this->_idShop, $attribute);
              }
              else{
                $combination = new Combination($attribute);
                if (property_exists($combination, 'location')) {
            ...
            if( method_exists(new StockAvailable(), 'setLocation') ){
              StockAvailable::setLocation((int)$product['id_product'], (string)$product['location'], $this->_idShop, 0);
            }
            else{
              if (property_exists($productObject, 'location')) {
        ...
        elseif( $field == "low_stock_threshold" ){
          if( trim($product['id_product_attribute']) ){
            ...
            if( $values != "" && $attribute ){
              $combination = new Combination($attribute);
              $combination->low_stock_threshold = (int)$values;
              if( ( $error = $combination->validateFields(false, true) ) !== true ){
                return $this->_createErrorsFile($error, $product['id_product']);
              }
              $combination->update();
            ...
            $productObject->low_stock_threshold = (int)$value;
          }
        }
        elseif( $field == "low_stock_alert" ){
          if( trim($product['id_product_attribute']) ){
            ...
            if( $values != "" && $attribute ){
              $combination = new Combination($attribute);
              $combination->low_stock_alert = (string)$values == '0' ? false : true;
              if( ( $error = $combination->validateFields(false, true) ) !== true ){
                return $this->_createErrorsFile($error, $product['id_product']);
              }
              $combination->update();
            ...
            $productObject->low_stock_alert = (string)$value == '0' ? false : true;
          }
        }
        elseif( $field == "available_date" ){

          if ( $product['available_date'] == '0000-00-00' || $product['available_date'] == "" ){
            continue;
          }
        ...
        elseif( $field == "combinations_mpn" ){
          $attribute =  (int)trim($product['id_product_attribute']);
          $values = trim($product['combinations_mpn']);
          if( $values && $attribute ){
            $combination = new Combination($attribute);
            $combination->mpn = $values;
            if( ( $error = $combination->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $combination->update();
        ...
        elseif( $field == "combinations_isbn" ){
          $attribute =  (int)trim($product['id_product_attribute']);
          $values = trim($product['combinations_isbn']);
          if( $values && $attribute ){
            $combination = new Combination($attribute);
            $combination->isbn = $values;
            if( ( $error = $combination->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $combination->update();
        ...
        elseif( $field == "specific_price_from_quantity" ){
          ...
          if( $values != "" ){
            ...
            foreach( $specific_price as $key => $specific_id ){
              $specificObject = new SpecificPrice((int)$specific_id);
        ...
        elseif( strpos($field, 'FEATURE_') !== false){
          ...
          foreach($featureValue as $key=>$fValue){
            ...
            $featureValueId = FeatureValue::addFeatureValueImport( (int)$featureId, $fValue, null, $this->_idLang );
            if( Module::getInstanceByName('pm_multiplefeatures')->active ){
              $this->_addFeatureProductImport( (int)$product['id_product'], (int)$featureId, (int)$featureValueId, $key );
            }
            else{
              $productObject->addFeatureProductImport( (int)$product['id_product'], (int)$featureId, (int)$featureValueId );
            }
          }
        }
        ...
        elseif( $field == "supplier_reference" ){
          ...
          $sId = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], (int)$attribute, (int)$productObject->id_supplier);
          if( $sId ){
            $sReference = new ProductSupplier($sId);
          }
          else{
            $sReference = new ProductSupplier();
            $sReference->id_product = (int)$product['id_product'];
            $sReference->id_product_attribute = (int)$attribute;
            $sReference->id_supplier = (int)$productObject->id_supplier;
          }
        ...
        elseif( $field == "supplier_price" ){
          ...
          if( $value != '' ){
            ...
            $sReference->product_supplier_price_te = $this->_preparePrice($value);
            if( ( $error = $sReference->validateFields(false, true) ) !== true ){
              return $this->_createErrorsFile($error, $product['id_product']);
            }
            $sReference->save();
        ...
        elseif( $field == "meta_keywords" ){
          if (property_exists($productObject, 'meta_keywords')) {
        ...
        elseif( $field == "suppliers_ids" ){
          ...
          foreach ($values as $id) {
            $id_product_supplier = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], 0, (int)$id);
            if ($id_product_supplier) {
              continue;
            }

            $supplierExists = Supplier::supplierExists((int)$id);
            if($supplierExists){
              $product_supplier = new ProductSupplier();
              $product_supplier->id_product = (int)$product['id_product'];
              $product_supplier->id_product_attribute = 0;
              $product_supplier->id_supplier = (int)$id;
              $product_supplier->save();
            }
          }
        }
        elseif( $field == "id_supplier" ){
          $supplierExists = Supplier::supplierExists((int)$value);
          if($supplierExists){
            $productObject->id_supplier = (int)$value;
            $product_supplier = new ProductSupplier();
            $product_supplier->id_product = (int)$product['id_product'];
            $product_supplier->id_product_attribute = 0;
            $product_supplier->id_supplier = (int)$value;
            $product_supplier->save();
          }
        }
        elseif( $field == "base_price" ){
          $productObject->price = $this->_preparePrice($value);
        }
        elseif( $field == "base_price_with_tax" ){
          ...
          $productObject->price = $this->_preparePrice($taxPrice);
        }
        elseif( $field == "unit_price" ){
          $productObject->unit_price = $this->_preparePrice($value);
        }
        elseif( $field == "wholesale_price" ){
          $productObject->wholesale_price = $this->_preparePrice($value);
        }
    ...
    private function _productImages($productId, $url_img, $combination = false, $image_caption = false){
      ...
      if(getimagesize($url_img)){
        ...
        $newpath_prod =  _PS_MODULE_DIR_ . 'updateproducts/upload/';
        ...
        if ( copy($url_img, $newpath_prod.$name_img) ) {
          ...
          {
            ImageManager::resize($newpath_prod.$name_img, $new_path.'-'. stripslashes($imageType['name']).'.'.$image->image_format, $imageType['width'], $imageType['height'], $image->image_format);
          }
    ...
    private function _updateProduct( $product )
    {
      ...
      foreach( $product as $field => $value ){
        ...
        elseif (preg_match('/^attribute_group_(\d+)$/', $field, $attribute_group_matches)) {
            if (!$value || empty($product['id_product_attribute'])) {
                continue;
            }

            $id_attribute_group = $attribute_group_matches[1];
            $attribute_names = explode(',', $value);
            $id_product_attributes = explode(';', $product['id_product_attribute']);

            foreach ($id_product_attributes as $key => $id_product_attribute) {
                if (!isset($attribute_names[$key])) {
                    continue;
                }

                $attribute_name = $attribute_names[$key];
                $this->updateAttribute($attribute_name, $id_attribute_group, $id_product_attribute);
            }
        }
        elseif($field == "pack_items_id"){
          ...
          if (!empty($attributes)) {
            ...
            if($values != "" ){
              ...
              foreach( $attributes as $key => $attribute ){
                ...
                $combination->price = $this->_preparePrice($values[$key]);
        ...
        elseif( $field == "combinations_final_price" ){
          ...
          if (empty($combination_ids) || empty($combination_final_prices)) {
              continue;
          }
          ...
          if (count($combination_ids) !== count($combination_final_prices)) {
              continue;
          }
          ...
          if (!empty($attributes)) {
            ...
            if($values != "" ){
              ...
              foreach( $attributes as $key => $attribute ){
                ...
                $combination->wholesale_price = $this->_preparePrice($values[$key]);
                if( ( $error = $combination->validateFields(false, true) ) !== true ){
                  return $this->_createErrorsFile($error, $product['id_product']);
                }
                $combination->update();
          ...
          if (!empty($attributes)) {
            ...
            if($values != "" ){
              ...
              foreach( $attributes as $key => $attribute ){
                ...
                $combination->unit_price_impact = $this->_preparePrice($values[$key]);
                if( ( $error = $combination->validateFields(false, true) ) !== true ){
                  return $this->_createErrorsFile($error, $product['id_product']);
                }
                $combination->update();
            ...
            if (!empty($attributes)) {
              ...
              if( $values != "" ){
                ...
                foreach( $attributes as $key => $attribute ){
                  ...
                  $combination->minimal_quantity = (int)$values[$key];
                  if( ( $error = $combination->validateFields(false, true) ) !== true ){
                    return $this->_createErrorsFile($error, $product['id_product']);
                  }
                  $combination->update();
            ...
            if (!empty($attributes)) {
              ...
              if( $values != "" ){
                ...
                foreach( $attributes as $key => $attribute ){
                  ...
                  if( method_exists(new StockAvailable(), 'setLocation') ){
                    StockAvailable::setLocation($product['id_product'], (string)$values[$key], $this->_idShop, (int)$attribute);
                  }
                  else{
                    $combination = new Combination((int)$attribute);
                    if (property_exists($combination, 'location')) {
            ...
            if (!empty($attributes)) {
              ...
              if( $values != "" ){
                ...
                foreach( $attributes as $key => $attribute ){
                  ...
                  $combination->low_stock_threshold = (int)$values[$key];
                  if( ( $error = $combination->validateFields(false, true) ) !== true ){
                    return $this->_createErrorsFile($error, $product['id_product']);
                  }
                  $combination->update();
            ...
            if (!empty($attributes)) {
              ...
              if( $values != "" ){
                ...
                foreach( $attributes as $key => $attribute ){
                  ...
                  $combination->low_stock_alert = (string)$values[$key] == '0' ? false : true;
                  if( ( $error = $combination->validateFields(false, true) ) !== true ){
                    return $this->_createErrorsFile($error, $product['id_product']);
                  }
                  $combination->update();
        ...
        elseif( $field == "combinations_mpn" ){
          ...
          if (!empty($attributes)) {
            ...
            $values = $product['combinations_mpn'];
            if( $values ){
              $values = str_replace(" ", "", $values);
              $values = explode(";", $values);
              foreach( $attributes as $key => $attribute ){
        ...
        elseif( $field == "combinations_isbn" ){
          ...
          if (!empty($attributes)) {
            $attributes = explode(";", $attributes);
            $values = $product['combinations_isbn'];
            if( $values ){
              $values = str_replace(" ", "", $values);
              $values = explode(";", $values);
              foreach( $attributes as $key => $attribute ){
          ...
          if( trim($product['id_product_attribute']) ){
            ...
            if( $values != "" ){
              ...
              foreach( $attributes as $key => $attribute ){
                ...
                $sId = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], (int)$attribute, (int)$productObject->id_supplier);
                if( $sId ){
                  $sReference = new ProductSupplier((int)$sId);
                }
                else{
                  $sReference = new ProductSupplier();
                  $sReference->id_product = (int)$product['id_product'];
            ...
            $sId = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], 0, (int)$productObject->id_supplier);
            if( $sId ){
              $sReference = new ProductSupplier((int)$sId);
            }
            else{
          ...
          if( trim($product['id_product_attribute']) ){
            ...
            if( $values != "" ){
              ...
              foreach( $attributes as $key => $attribute ){
                ...
                $sReference->product_supplier_price_te = $this->_preparePrice($values[$key]);
                if( ( $error = $sReference->validateFields(false, true) ) !== true ){
                  return $this->_createErrorsFile($error, $product['id_product']);
                }
                $sReference->save();
    ...
    private function _copyFile()
    {
      ...
      if (!Tools::copy($_FILES['file']['tmp_name'],  _PS_MODULE_DIR_ . 'updateproducts/files/import_products.'. $this->_format)){
        throw new Exception(Module::getInstanceByName('updateproducts')->l('An error occurred while uploading file!', 'import') );
      }

      if ($this->_format == 'csv'){
        $reader = PHPExcel_IOFactory::createReader("CSV");
        $file_encoding = mb_detect_encoding(Tools::file_get_contents(_PS_MODULE_DIR_.'updateproducts/data/import_products.csv'), array('UTF-8','ISO-8859-1','ASCII','GBK'), true);

        if ($file_encoding) {
           /** @var \PHPExcel_Reader_CSV $reader */
          $reader->setInputEncoding($file_encoding);
        }

        $this->_PHPExcelFactory = $reader->load(_PS_MODULE_DIR_ . "updateproducts/files/import_products." . $this->_format );
      } else {
        $this->_PHPExcelFactory = PHPExcel_IOFactory::load(_PS_MODULE_DIR_ . "updateproducts/files/import_products." . $this->_format );
      }
    }


    private function _createErrorsFile($error, $id_product)
    {

      $write_fd = fopen(_PS_MODULE_DIR_ . 'updateproducts/error/error_logs.csv', 'a+');
      if (@$write_fd !== false){
        fwrite($write_fd, $id_product . ',' . $error . "\r\n");
      }
      fclose($write_fd);
    ...
    {
      $all_shops = $this->_context->shop->getContext() == Shop::CONTEXT_ALL ? true : false;
      ...
      $features = Db::getInstance()->executeS('
        ...
        .(!$all_shops ? ' AND fs.`id_shop` = '.(int)$this->_context->shop->id : '') .
    ...
    private function updateProductSupplierProperty($product, $property_name)
    {
      ...
      foreach ($properties_grouped_by_suppliers as $property_key => $properties) {
        ...
        foreach ($combinations as $key => $id_product_attribute) {
          $product_supplier_id = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], (int)$id_product_attribute, (int)$product_suppliers_ids[$property_key]);
          ...
          if($product_supplier_id){
            ...
            $product_supplier->id_supplier = (int)$product_suppliers_ids[$property_key];
          }
          ...
          switch ($property_name) {
            ...
            case 'suppliers_price':
              $product_supplier->product_supplier_price_te = $this->_preparePrice($properties_ready_for_update[$key]);
              break;
    ...
    private function updateProductSupplierPropertySeparate($product, $property_name)
    {
      ...
      foreach ($properties_grouped_by_suppliers as $property_key => $property) {
        ...
        $product_supplier_id = ProductSupplier::getIdByProductAndSupplier((int)$product['id_product'], (int)$product['id_product_attribute'], (int)$product_suppliers_ids[$property_key]);
        ...
        if ($product_supplier_id) {
          ...
          $product_supplier = new ProductSupplier();
          $product_supplier->id_product = (int)$product['id_product'];
          $product_supplier->id_product_attribute = (int)$product['id_product_attribute'];
          $product_supplier->id_supplier = (int)$product_suppliers_ids[$property_key];
        }
        ...
        switch ($property_name) {
          case 'suppliers_reference':
            ...
            break;
          case 'suppliers_price':
            $product_supplier->product_supplier_price_te = $this->_preparePrice($property);
            break;
          case 'suppliers_price_currency':
    ...
    private function updateAttribute($attribute_name, $id_attribute_group, $id_product_attribute)
    {
      $attribute_id = $this->getAttributeIdByName($attribute_name, $id_attribute_group);
      $all_combination_attribute_ids = $this->getAllCombinationAttributeIdsAsString($id_product_attribute);

      if (!$attribute_id || empty($all_combination_attribute_ids)) {
          return false;
      }

      foreach ($all_combination_attribute_ids as &$combination_attribute_id) {
        if (version_compare(_PS_VERSION_, '1.8', '<') == true) {
          $attribute_obj = new Attribute($combination_attribute_id);
        }
        else {
          $attribute_obj = new ProductAttribute($combination_attribute_id);
        }

          if ($attribute_obj->id_attribute_group == $id_attribute_group) {
              $combination_attribute_id = $attribute_id;
              break;
          }
      }

      return $this->setAttributeForCombination($id_product_attribute, $all_combination_attribute_ids);
    }

    private function setAttributeForCombination($id_product_attribute, $idsAttribute)
    {
      $result = $this->deleteCombinationAssociations($id_product_attribute);

      if ($result && !empty($idsAttribute)) {
          $sqlValues = array();
          foreach ($idsAttribute as $value) {
              $sqlValues[] = '(' . (int) $value . ', ' . (int)$id_product_attribute . ')';
          }
          ...
          if ($result) {
              Hook::exec('actionAttributeCombinationSave', array('id_product_attribute' => (int)$id_product_attribute, 'id_attributes' => $idsAttribute));
          }
      }
    ...
    private function deleteCombinationAssociations($id_product_attribute)
    {
        $result = Db::getInstance()->delete('product_attribute_combination', '`id_product_attribute` = ' . (int)$id_product_attribute);
        $result &= Db::getInstance()->delete('cart_product', '`id_product_attribute` = ' . (int)$id_product_attribute);

        if ($result) {
            Hook::exec('actionAttributeCombinationDelete', array('id_product_attribute' => (int)$id_product_attribute));
        }

        return $result;
    }

    private function getAttributeIdByName($attribute_name, $id_attribute_group)
    {
      return Db::getInstance()->getValue("SELECT a.id_attribute FROM " . _DB_PREFIX_ . "attribute as a
                                                        LEFT JOIN " . _DB_PREFIX_ . "attribute_lang as al
                                                        ON a.id_attribute = al.id_attribute
                                                        WHERE a.id_attribute_group = '".(int)$id_attribute_group."'
                                                        AND al.id_lang = '".(int)$this->_idLang."'
                                                        AND al.name = '".pSQL($attribute_name)."'"
      );
    }

    private function getAllCombinationAttributeIdsAsString($id_product_attribute)
    {
      $result = Db::getInstance()->getValue("SELECT GROUP_CONCAT(id_attribute) FROM " . _DB_PREFIX_ . "product_attribute_combination 
                                                WHERE id_product_attribute = '".(int)$id_product_attribute."'");

      return ($result ? explode(',', $result) : []);
    }

    private function updateCategories(Product $product, $category_ids)
    {
      ...
      if (!$product->updateCategories($category_ids)) {
          return false;
      }
    ...
    private function isProductExists($id_product)
    {
