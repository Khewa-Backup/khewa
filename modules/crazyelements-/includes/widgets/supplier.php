...
class Widget_Supplier extends Widget_Base {
	...
	private function get_suppliers_list_array() {
		...
		foreach ( $rs as $key => $value ) {
			$rslt[ $value['id_supplier'] ] = $value['name'];
		}
		return $rslt;
	}
	...
	protected function render() {
		...
		$output  = '';
		$str             = '';
		$id_supplier = explode('_',$id_supplier);
		if(isset($id_supplier) && is_array($id_supplier)){
			$str = $id_supplier[0];
		}else{
			$str             = '';
		}
		if ( $str != '' ) {
			...
			$products = \Supplier::getProducts( (int) $str, $context->language->id, $page, $per_page, $orderby, $order );
			if ( ! $products ) {
				return false;
			}
			...
			$meta_tags = \Meta::getMetaTags( $context->language->id, $page_name );
			$page      = array(
				'title'               => '',
				'canonical'           => '',
				'meta'                => array(
