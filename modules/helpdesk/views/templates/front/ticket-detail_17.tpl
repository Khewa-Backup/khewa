...
<br />
<div class="block-center page-order-detail" id="block-history">
	...
	{if $ticketResponsesData}
		<div class="block-center box row" id="block-history">
			<div class="separation"></div>
			<div class="panel card">
			<h3 class="card-header">{l s='Ticket Responses' mod='helpdesk'}</h3>	
			<div class="separation"></div>
				{if !empty($ticketResponsesData)}<div class="card-body">{/if}
				{foreach from=$ticketResponsesData item=ticketResponse}
				
				<div class="fmm_help_singular_reply row{if isset($ticketResponse.r_client_id) && $ticketResponse.r_client_id <= 0} fmm_helpdesk_admin_reply_badge{/if}">
					<div class="message-item col-lg-12">
							<div class="fmm_helpdesk_thumb_holder_101">
						...
						<div class="message-body fmm_helpdesk_messagebody_wrapper">
						<span class="message-date fmm_helpdesk_dates">&nbsp;
								{if isset($its_ps17)}<i class="material-icons">event</i>{else}<i class="icon-calendar"></i>{/if} - 
						...
						{if $ticketResponse.r_attachment}
							<span class="message-product">&nbsp;
								{if isset($its_ps17)}<i class="material-icons">link</i>{else}<i class="icon-link"></i>{/if} <a href="{$path|escape:'htmlall':'UTF-8'}img/{$ticketResponse.r_attachment|escape:'htmlall':'UTF-8'}" target="_blank" class="_blank">{l s='Attachment' mod='helpdesk'}</a></span>
						{/if}
						<p class="fmm_helpdesk_reply_txts">{$ticketResponse.r_message|escape:'htmlall':'UTF-8'}</p>
						
						</div>
					</div>
				</div>
				{/foreach}
				{if !empty($ticketResponsesData)}</div>{/if}
		</div>
	{else}
...
{if isset($confirmation) && $confirmation}
	<p class="success col-xs-12 alert alert-success">
...
<div class="box">
<form action="{$link->getModuleLink('helpdesk', 'helpdesk')|escape:'htmlall':'UTF-8'}{if $seo_url == 0}&{else}?{/if}ticket_id={$ticketData[0].ticket_id|escape:'htmlall':'UTF-8'}&detail=1" method="post" class="std" id="add_address" name="helpdesk_form" enctype="multipart/form-data">
		...
		<div class="submit2">
			...
			<input type="submit" class="button btn btn-primary" value="{l s='Submit' mod='helpdesk'}" id="submitAddress" name="submitAddress">
		</div>
</form>
</div>
