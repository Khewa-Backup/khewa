...
class AdminEtsyImportController extends ModuleAdminController
{
    ...
    public function showProductUrl($row_data, $tr)
    {
        ...
        $url = $this->context->link->getProductLink($product_id);
    ...
    public function renderList()
    {
        ...
        $instrctions = $this->context->smarty->fetch(_PS_MODULE_DIR_ . "kbetsy/views/templates/admin/msgs.tpl");
        ...
        if (!empty($remaining_limit)) {
            ...
            return $msgs . parent::renderList().$instrctions;
        } else {
            return parent::renderList().$instrctions;
        }
    }
...
//            Db::getInstance()->query('UPDATE ' . _DB_PREFIX_ . 'etsy_products_list SET active = "' . (int) $final_status . '" WHERE id_product = ' . (int) $profile_product[0]['id_product']);
