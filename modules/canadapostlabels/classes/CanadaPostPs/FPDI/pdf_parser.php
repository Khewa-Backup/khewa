...
class pdf_parser
{
    ...
    protected function _findXref()
    {
        ...
        $pos = \Tools::strlen($data) - $keywordPos;
        $data = \Tools::substr($data, $pos);
    ...
    protected function _readXref(&$result, $offset)
    {
        ...
        $data = ltrim(\Tools::substr($data, 0, $trailerPos));

        // get Line-Ending
        $found = preg_match_all("/(\r\n|\n|\r)/", \Tools::substr($data, 0, 100), $m); // check the first 100 bytes for line breaks
        if ($found === 0) {
        ...
        switch ($token) {
            case '<':
                ...
                while (1) {
                    ...
                    $result = \Tools::substr($c->buffer, $c->offset, $match - $c->offset);
                    $c->offset = $match + 1;
            ...
            case '(':
                ...
                $result = \Tools::substr($c->buffer, $c->offset, $pos - $c->offset - 1);
                $c->offset = $pos;
    ...
    protected function _readToken($c)
    {
        ...
        switch ($char) {
            ...
            default:
                ...
                $result = \Tools::substr($c->buffer, $c->offset - 1, $pos + 1);
                ...
                case '/Fl':
                    ...
                    if ($stream === false) {
                        ...
                        while ($tries < 8 && ($stream === false || strlen($stream) < strlen($oStream))) {
                            $oStream = \Tools::substr($oStream, 1);
