...
class ElegantalSeoEssentials extends ElegantalSeoEssentialsModule
{
    ...
    public function getContent()
    {
        ...
        $this->context->controller->addCSS($this->_path . 'views/css/elegantalseoessentials-back6.css', 'all');
        $this->context->controller->addJqueryUI('ui.sortable');
        $this->context->controller->addJS($this->_path . 'views/js/elegantalseoessentials-back6.js');
        ...
        $fields_form = array(
            'form' => array(
                ...
                'submit' => array(
                    'title' => $this->l('Save'),
                    'name' => 'submitAndNext',
                    'class' => 'btn btn-default pull_right',
                ),
                'buttons' => array(
                    array(
    ...
    protected function metaTagsUpdate()
    {
        ...
        $category_input = array(
            ...
            'desc' => $this->l('Select categories for which you want to apply this rule.') . ' ' . $this->l('You can leave it empty to apply the rule to all categories.'),
        );
        if (_PS_VERSION_ < '1.6') {
            $category_input = array(
                'type' => 'categories',
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'desc' => $this->l('Choose product properties to insert into pattern.') . ' ' . $this->l('You can add custom words too.') . ' ' . $this->l('Leave empty to keep product URL unchanged.') . ' ' . $this->l('NOTE') . ': ' . $this->l('Friendly URL must be enabled for this feature.') . ' ' . $this->l('You may set length for each product attribute in the following format:') . ' {product_name|50}',
                    ),
                    array(
                        'type' => 'text',
                        ...
                        'lang' => true,
                        'class' => 'elegantaltagpopover',
                        'desc' => $this->l('Choose product properties to insert into pattern.') . ' ' . $this->l('You can add custom words too.') . ' ' . $this->l('Leave empty to keep product page title unchanged.') . ' ' . $this->l('You may set length for each product attribute in the following format:') . ' {product_name|50}',
                    ),
                    array(
                        'type' => 'text',
                        ...
                        'lang' => true,
                        'class' => 'elegantaltagpopover',
                        'desc' => $this->l('Choose product properties to insert into pattern.') . ' ' . $this->l('You can add custom words too.') . ' ' . $this->l('Leave empty to keep product page meta description unchanged.') . ' ' . $this->l('You may set length for each product attribute in the following format:') . ' {product_name|50}',
                    ),
                    array(
    ...
    protected function metaTagsApply()
    {
        ...
        $limit = ($limit > 0 && $limit < 1000000) ? $limit : 100;
        $requests = 1;
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'lang' => true,
                        'class' => 'elegantaltagpopover',
                        'desc' => $this->l('Choose product properties to insert into pattern.') . ' ' . $this->l('You can add custom words too.') . ' ' . $this->l('You may set length for each product attribute in the following format:') . ' {product_name|50}',
                    ),
                    array(
                        'type' => (_PS_VERSION_ < '1.6') ? 'el_switch' : 'switch',
                        'label' => $this->l('Active'),
    ...
    protected function getHooksForSelect()
    {
        ...
        $hooks = array(
            ...
            array('key' => 'displayBackOfficeHeader', 'value' => 'displayBackOfficeHeader'),
            ...
            'form' => array(
                ...
                'buttons' => array(
                    ...
                    array(
                        ...
                        'title' => $this->l('Custom Canonical URL'),
                        'class' => 'pull_right',
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => (_PS_VERSION_ < '1.6') ? 'el_switch' : 'switch',
                        'label' => $this->l('Include Multishop Hreflang Tags'),
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'desc' => $this->l('Enter existing URL within your shop which you want to redirect to another URL.') . ' ' . $this->l('NOTE') . ': ' . $this->l('You should enter URL without domain of your shop and it should start with /'),
                    ),
                    array(
                        'type' => 'text',
    ...
    protected function redirectsImport()
    {
        ...
        if ($this->isPostRequest()) {
            ...
            // Validate mime type
            $allowed_mime_types = array('text/plain', 'text/csv');
            $extension = Tools::strtolower(pathinfo($_FILES['csv_file']['name'], PATHINFO_EXTENSION));
            $mime = ElegantalSeoEssentialsTools::getMimeType($_FILES['csv_file']["tmp_name"], $extension);
            if (!in_array($mime, $allowed_mime_types)) {
                $errors[] = $this->l('This file type is not allowed.') . ' Mime Type: ' . $mime . '. ' . $this->l('You may import only CSV.');
            }
    ...
    protected function redirectsExport()
    {
        ...
        $file = ElegantalSeoEssentialsTools::getTempDir() . '/' . $filename;
            ...
            'form' => array(
                ...
                'input' => array(
                    array(
                        ...
                        'prefix' => $shop_url,
                        'placeholder' => '/example/old-url',
                        'desc' => $this->l('Enter existing URL within your shop for which you want to create custom canonical URL.') . ' ' . $this->l('NOTE') . ': ' . $this->l('You should enter URL without domain of your shop and it should start with /'),
                    ),
                    array(
                        'type' => 'text',
                        'label' => $this->l('New URL'),
                        ...
                        'desc' => $this->l('Enter New URL that you want to use as canonical URL instead of Old URL above.') . ' ' . $this->l('NOTE') . ': ' . $this->l('You should enter URL without domain of your shop and it should start with /'),
                    ),
                    array(
                        'type' => (_PS_VERSION_ < '1.6') ? 'el_switch' : 'switch',
                        'label' => $this->l('Active'),
    ...
    protected function getCanonicalUrl($add_pagination_param = true)
    {
        ...
        if ($canonical) {
            ...
            $canonical_url = $context->link->getProductLink($id_product, null, null, null, $id_lang, $id_shop, $id_product_attribute, false, false, false, [], false);
            ...
            $canonical_url = call_user_func(array('AdvancedTag', 'getModuleLink'), 'advancedtags', 'browse', array('tag' => Tools::strtolower(Tools::getValue('tag'))), true, $id_lang);
    ...
    protected function getHreflangs($use_multishop_domain = false)
    {
        ...
        foreach ($languages as $language) {
            ...
            if (Tools::getValue('fc') == 'module' && Tools::getValue('module') == 'pm_advancedsearch4' && class_exists('AdvancedSearchSeoClass')) {
                $AdvancedSearchSeoClass = 'AdvancedSearchSeoClass';
                $ObjAdvancedSearchSeoClass = new $AdvancedSearchSeoClass((int) Tools::getValue('id_seo'), null);
                $hreflangs[$language['language_code']] = $hreflangs[$language['iso_code']] = $this->context->link->getModuleLink('pm_advancedsearch4', 'seo', array('id_seo' => (int) $ObjAdvancedSearchSeoClass->id, 'seo_url' => $ObjAdvancedSearchSeoClass->seo_url[$language['id_lang']]), null, $language['id_lang']);
            } elseif (Tools::getValue('fc') == 'module' && Tools::getValue('module') == 'advancedtags' && $controller == 'browse' && method_exists('AdvancedTag', 'getModuleLink')) {
                $hreflangs[$language['language_code']] = $hreflangs[$language['iso_code']] = call_user_func(array('AdvancedTag', 'getModuleLink'), 'advancedtags', 'browse', array('tag' => Tools::strtolower(Tools::getValue('tag'))), true, $language['id_lang']);
            } elseif ($controller == 'product' && Tools::getValue('id_product')) {
                $id_product = (int) Tools::getValue('id_product');
                $id_product_attribute = (int) Tools::getValue('id_product_attribute');
                $hreflangs[$language['language_code']] = $hreflangs[$language['iso_code']] = $this->context->link->getProductLink($id_product, null, null, null, $language['id_lang'], $this->context->shop->id, $id_product_attribute, false, false, false, [], false);
            } else {
                $hreflangs[$language['language_code']] = $hreflangs[$language['iso_code']] = $this->context->link->getLanguageLink($language['id_lang'], $this->context);
            }
        }
                ...
                foreach ($shop_group['shops'] as $shop) {
                    ...
                    foreach ($shop_languages as $shlang) {
                        if (Tools::getValue('fc') == 'module' && Tools::getValue('module') == 'pm_advancedsearch4' && class_exists('AdvancedSearchSeoClass')) {
                            $AdvancedSearchSeoClass = 'AdvancedSearchSeoClass';
                            $ObjAdvancedSearchSeoClass = new $AdvancedSearchSeoClass((int) Tools::getValue('id_seo'), null);
                            $hreflangs[$shlang['language_code']] = $hreflangs[$shlang['iso_code']] = $tmpContext->link->getModuleLink('pm_advancedsearch4', 'seo', array('id_seo' => (int) $ObjAdvancedSearchSeoClass->id, 'seo_url' => $ObjAdvancedSearchSeoClass->seo_url[$shlang['id_lang']]), null, $shlang['id_lang']);
                        } elseif (Tools::getValue('fc') == 'module' && Tools::getValue('module') == 'advancedtags' && $controller == 'browse' && method_exists('AdvancedTag', 'getModuleLink')) {
                            $hreflangs[$shlang['language_code']] = $hreflangs[$shlang['iso_code']] = call_user_func(array('AdvancedTag', 'getModuleLink'), 'advancedtags', 'browse', array('tag' => Tools::strtolower(Tools::getValue('tag'))), true, $shlang['id_lang']);
                        } elseif ($controller == 'product' && Tools::getValue('id_product')) {
                            $id_product = (int) Tools::getValue('id_product');
                            $id_product_attribute = (int) Tools::getValue('id_product_attribute');
                            $hreflangs[$shlang['language_code']] = $hreflangs[$shlang['iso_code']] = $tmpContext->link->getProductLink($id_product, null, null, null, $shlang['id_lang'], $shop['id_shop'], $id_product_attribute, false, false, false, [], false);
                        } else {
                            $hreflangs[$shlang['language_code']] = $hreflangs[$shlang['iso_code']] = $tmpContext->link->getLanguageLink($shlang['id_lang'], $tmpContext);
                        }
                    }
                }
            }
    ...
    protected function getNextPrevTags()
    {
        ...
        if ($controller == "category" && Tools::getValue('id_category')) {
            ...
            if (ElegantalSeoEssentialsTools::isModuleInstalled('pm_advancedsearch4') && class_exists('PM_AdvancedSearch4')) {
            ...
            $id_tag = call_user_func(array('AdvancedTag', 'getTagIdByName'), Tools::getValue('tag'), $id_lang);
            $total_items = (int) call_user_func(array('AdvancedTag', 'searchTag'), $id_lang, (int) $id_tag, true, 0, 10, false, false);
            $url = call_user_func(array('AdvancedTag', 'getModuleLink'), 'advancedtags', 'browse', array('tag' => Tools::strtolower(Tools::getValue('tag'))), true, $id_lang);
        }
