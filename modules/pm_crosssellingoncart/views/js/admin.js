...
function initProductsAutocomplete()
{
	$('#product_autocomplete_input').autocomplete('ajax_products_list.php', {
			...
			formatItem: function(item) {
				item[2] = '<div style="margin-right: 10px;float:left;"><img src="../img/tmp/product_mini_' + item[1] + '_' + idShop + '.jpg?time='+Date.now()+'" /></div>';
...
function addProduct(event, data, formatted)
{
	...
	$divProducts.html($divProducts.html() + '<div id="csoc_product_'+ productId +'" class="form-control-static"><button type="button" class="btn btn-default" onclick="delProduct('+ productId +')" name="' + productId + '"><i class="icon-remove text-danger"></i></button><img src="../img/tmp/product_mini_' + productId + '_' + idShop + '.jpg?time='+Date.now()+'" />&nbsp;'+ productName +'</div>');
...
function delProduct(id)
{
	...
	for (i in inputCut)
	{
		...
		if (inputCut[i] != id)
		{
			...
			div.innerHTML += '<div class="form-control-static"><button type="button" class="btn btn-default" onclick="delProduct('+ inputCut[i] +')" name="' + inputCut[i] +'"><i class="icon-remove text-danger"></i></button><img src="../img/tmp/product_mini_' + inputCut[i] + '_' + idShop + '.jpg?time='+Date.now()+'" />&nbsp;' + nameCut[i] + '</div>';
		}
		else
