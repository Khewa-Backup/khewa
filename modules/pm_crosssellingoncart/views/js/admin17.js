...
function initProductsAutocomplete()
{
	...
	var queryParams = 'forceJson=1';
	// If the product autocompletion token is defined, we are on PS 1.7.6, which requires a token for ajax_products_list.php calls
	if (typeof (pm_crosssellingoncart.productsAutocompleteToken) !== 'undefined') {
		queryParams = queryParams + '&token=' + pm_crosssellingoncart.productsAutocompleteToken;
	}

	$('#product_autocomplete_input').autocomplete('ajax_products_list.php?' + queryParams, {
			minLength: 2,
			minChars: 1,
